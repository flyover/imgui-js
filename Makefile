...
start-example-html:
	@echo "open http://localhost:8080/example/index.html"
	$$(npm bin)/http-server
...
#	windows
#	set SDL2DIR=C:\SDL2 && cl /Zi /MD /I $(IMGUI_NATIVE_EXAMPLE_PATH) /I $(IMGUI_PATH) /I %SDL2DIR%\include $(IMGUI_NATIVE_EXAMPLE_SOURCE_CXX) $(IMGUI_SOURCE_CXX) /link /LIBPATH:%SDL2DIR%\lib SDL2.lib SDL2main.lib opengl32.lib /subsystem:console /Fe $(IMGUI_NATIVE_EXAMPLE_PATH)/sdl2example-windows.exe
#	linux
	c++ `sdl2-config --cflags` -I $(IMGUI_NATIVE_EXAMPLE_PATH) -I $(IMGUI_PATH) -D HAVE_MALLINFO $(IMGUI_NATIVE_EXAMPLE_SOURCE_CXX) $(IMGUI_SOURCE_CXX) `sdl2-config --libs` -lGL -o $(IMGUI_NATIVE_EXAMPLE_PATH)/sdl2example-linux
#	macos
#	c++ `sdl2-config --cflags` -I $(IMGUI_NATIVE_EXAMPLE_PATH) -I $(IMGUI_PATH) $(IMGUI_NATIVE_EXAMPLE_SOURCE_CXX) $(IMGUI_SOURCE_CXX) `sdl2-config --libs` -framework OpenGl -o $(IMGUI_NATIVE_EXAMPLE_PATH)/sdl2example-macos

clean-native-example:
#	rm -f $(IMGUI_NATIVE_EXAMPLE_PATH)/sdl2example-windows
	rm -f $(IMGUI_NATIVE_EXAMPLE_PATH)/sdl2example-linux
#	rm -f $(IMGUI_NATIVE_EXAMPLE_PATH)/sdl2example-macos

start-native-example:
#	$(IMGUI_NATIVE_EXAMPLE_PATH)/sdl2example-windows
	$(IMGUI_NATIVE_EXAMPLE_PATH)/sdl2example-linux
#	$(IMGUI_NATIVE_EXAMPLE_PATH)/sdl2example-macos
