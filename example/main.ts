...
import * as ImGui_Impl from "../src/imgui_impl_webgl";
...
const clear_color: ImVec4 = new ImVec4(0.45, 0.55, 0.6, 1.0);
...
async function AddFontFromFileTTF(
  url: string,
  size_pixels: number,
  font_cfg: ImGui.ImFontConfig | null = null,
  glyph_ranges: number | null = null
): Promise<ImGui.ImFont> {
  font_cfg = font_cfg || new ImGui.ImFontConfig();
  font_cfg.Name =
    font_cfg.Name ||
    `${url.split(/[\\\/]/).pop()}, ${size_pixels.toFixed(0)}px`;
  return ImGui.GetIO().Fonts.AddFontFromMemoryTTF(
    await LoadArrayBuffer(url),
    size_pixels,
    font_cfg,
    glyph_ranges
  );
...
function _loop(time: number): void {
  ...
  // 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
  {
    ...
    if (ImGui.ImageButton(image_gl_texture, new ImVec2(48, 48))) {
      ...
      image_url =
        image_urls[(image_urls.indexOf(image_url) + 1) % image_urls.length];
      if (image_element) {
    ...
    if (show_gamepad_window)
      ShowGamepadWindow(
        "Gamepad Window",
        (value = show_gamepad_window) => (show_gamepad_window = value)
      );
    ImGui.SameLine();
    ...
    if (show_movie_window)
      ShowMovieWindow(
        "Movie Window",
        (value = show_movie_window) => (show_movie_window = value)
      );
  ...
  if (ctx) {
    ...
    ctx.fillStyle = `rgba(${clear_color.x * 0xff}, ${clear_color.y * 0xff}, ${
      clear_color.z * 0xff
    }, ${clear_color.w})`;
...
async function _done(): Promise<void> {
  ...
  console.log(
    "Total allocated space (uordblks) @ _done:",
    ImGui.bind.mallinfo().uordblks
  );
}
...
function ShowSandboxWindow(
  title: string,
  p_open: ImGui.ImAccess<boolean> | null = null
): void {
...
function UpdateVideo(): void {
  ...
  if (
    gl &&
    video_element &&
    video_element.readyState >= video_element.HAVE_CURRENT_DATA
  ) {
    gl.bindTexture(gl.TEXTURE_2D, video_gl_texture);
    gl.texImage2D(
      gl.TEXTURE_2D,
  ...
  if (video_element !== null) {
    ...
    if (
      ImGui.BeginCombo(
        "##urls",
        null,
        ImGui.ComboFlags.NoPreview | ImGui.ComboFlags.PopupAlignLeft
      )
    ) {
    ...
    ImGui.SliderFloat(
      "##time",
      (value = video_time) => (video_time = value),
      0,
      video_duration
    );
