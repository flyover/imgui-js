...
function canvas_on_keydown(event: KeyboardEvent): void {
  ...
  ImGui.IM_ASSERT(
    event.keyCode >= 0 && event.keyCode < ImGui.IM_ARRAYSIZE(io.KeysDown)
  );
...
export function Init(
  ...
  if (typeof window !== "undefined") {
    ...
    ImGui.LoadIniSettingsFromMemory(
      window.localStorage.getItem("imgui.ini") || ""
    );
  }
...
export function NewFrame(time: number): void {
  ...
  if (io.ConfigFlags & ImGui.ConfigFlags.NavEnableGamepad) {
    ...
    for (let i = 0; i < gamepads.length; ++i) {
      ...
      const match: RegExpMatchArray | null = gamepad.id.match(
        /^([0-9a-f]{4})-([0-9a-f]{4})-.*$/
      );
      ...
      const vendor: string =
        (match && match[1]) || (match_chrome && match_chrome[1]) || "0000";
      const product: string =
        (match && match[2]) || (match_chrome && match_chrome[2]) || "0000";
...
export function RenderDrawData(
  draw_data: ImGui.ImDrawData | null = ImGui.GetDrawData()
): void {
  const io = ImGui.GetIO();
  if (draw_data === null) {
    throw new Error();
  ...
  // Backup GL state
  const last_active_texture: GLenum | null =
    (gl && gl.getParameter(gl.ACTIVE_TEXTURE)) || null;
  const last_program: WebGLProgram | null =
    (gl && gl.getParameter(gl.CURRENT_PROGRAM)) || null;
  const last_texture: WebGLTexture | null =
    (gl && gl.getParameter(gl.TEXTURE_BINDING_2D)) || null;
  const last_array_buffer: WebGLBuffer | null =
    (gl && gl.getParameter(gl.ARRAY_BUFFER_BINDING)) || null;
  const last_element_array_buffer: WebGLBuffer | null =
    (gl && gl.getParameter(gl.ELEMENT_ARRAY_BUFFER_BINDING)) || null;
  // GLint last_polygon_mode[2]; glGetIntegerv(GL_POLYGON_MODE, last_polygon_mode);
  const last_viewport: Int32Array | null =
    (gl && gl.getParameter(gl.VIEWPORT)) || null;
  const last_scissor_box: Int32Array | null =
    (gl && gl.getParameter(gl.SCISSOR_BOX)) || null;
  const last_blend_src_rgb: GLenum | null =
    (gl && gl.getParameter(gl.BLEND_SRC_RGB)) || null;
  const last_blend_dst_rgb: GLenum | null =
    (gl && gl.getParameter(gl.BLEND_DST_RGB)) || null;
  const last_blend_src_alpha: GLenum | null =
    (gl && gl.getParameter(gl.BLEND_SRC_ALPHA)) || null;
  const last_blend_dst_alpha: GLenum | null =
    (gl && gl.getParameter(gl.BLEND_DST_ALPHA)) || null;
  const last_blend_equation_rgb: GLenum | null =
    (gl && gl.getParameter(gl.BLEND_EQUATION_RGB)) || null;
  const last_blend_equation_alpha: GLenum | null =
    (gl && gl.getParameter(gl.BLEND_EQUATION_ALPHA)) || null;
  const last_enable_blend: GLboolean | null =
    (gl && gl.getParameter(gl.BLEND)) || null;
  const last_enable_cull_face: GLboolean | null =
    (gl && gl.getParameter(gl.CULL_FACE)) || null;
  const last_enable_depth_test: GLboolean | null =
    (gl && gl.getParameter(gl.DEPTH_TEST)) || null;
  const last_enable_scissor_test: GLboolean | null =
    (gl && gl.getParameter(gl.SCISSOR_TEST)) || null;
  ...
  gl &&
    ...
    gl.uniformMatrix4fv(g_AttribLocationProjMtx, false, ortho_projection);
  ...
  gl &&
    gl.vertexAttribPointer(
      ...
      true,
      ImGui.ImDrawVertSize,
      ImGui.ImDrawVertColOffset
    );

  // Draw
  const pos = draw_data.DisplayPos;
  const idx_buffer_type: GLenum =
    (gl && (ImGui.ImDrawIdxSize === 4 ? gl.UNSIGNED_INT : gl.UNSIGNED_SHORT)) ||
    0;
    ...
    draw_list.IterateDrawCmds((draw_cmd: ImGui.ImDrawCmd): void => {
      ...
      if (!gl && !ctx) {
        ...
        for (let i = 0; i < Math.min(3, draw_cmd.ElemCount); ++i) {
          const view: ImGui.ImDrawVert = new ImGui.ImDrawVert(
            draw_list.VtxBuffer.buffer,
            draw_list.VtxBuffer.byteOffset + i * ImGui.ImDrawVertSize
          );
          console.log(
            `${i}: ${view.pos[0].toFixed(2)} ${view.pos[1].toFixed(
              2
            )} ${view.uv[0].toFixed(5)} ${view.uv[1].toFixed(5)} ${(
              "00000000" + view.col[0].toString(16)
            ).substr(-8)}`
          );
        }
      }
      ...
      if (draw_cmd.UserCallback !== null) {
        ...
        const clip_rect = new ImGui.ImVec4(
        ...
        if (
          ...
          if (ctx) {
            ...
            ctx.clip();
            const idx =
              ImGui.ImDrawIdxSize === 4
                ? new Uint32Array(
                    draw_list.IdxBuffer.buffer,
                    draw_list.IdxBuffer.byteOffset + idx_buffer_offset
                  )
                : new Uint16Array(
                    draw_list.IdxBuffer.buffer,
                    draw_list.IdxBuffer.byteOffset + idx_buffer_offset
            ...
            for (let i = 0; i < draw_cmd.ElemCount; i += 3) {
              ...
              const v0: ImGui.ImDrawVert = new ImGui.ImDrawVert(
                draw_list.VtxBuffer.buffer,
                draw_list.VtxBuffer.byteOffset + i0 * ImGui.ImDrawVertSize
              );
              const v1: ImGui.ImDrawVert = new ImGui.ImDrawVert(
                draw_list.VtxBuffer.buffer,
                draw_list.VtxBuffer.byteOffset + i1 * ImGui.ImDrawVertSize
              );
              const v2: ImGui.ImDrawVert = new ImGui.ImDrawVert(
                draw_list.VtxBuffer.buffer,
                draw_list.VtxBuffer.byteOffset + i2 * ImGui.ImDrawVertSize
              );
              const i3: number = idx[i + 3];
              const i4: number = idx[i + 4];
              const i5: number = idx[i + 5];
              const v3: ImGui.ImDrawVert = new ImGui.ImDrawVert(
                draw_list.VtxBuffer.buffer,
                draw_list.VtxBuffer.byteOffset + i3 * ImGui.ImDrawVertSize
              );
              const v4: ImGui.ImDrawVert = new ImGui.ImDrawVert(
                draw_list.VtxBuffer.buffer,
                draw_list.VtxBuffer.byteOffset + i4 * ImGui.ImDrawVertSize
              );
              const v5: ImGui.ImDrawVert = new ImGui.ImDrawVert(
                draw_list.VtxBuffer.buffer,
                draw_list.VtxBuffer.byteOffset + i5 * ImGui.ImDrawVertSize
              );
              let quad = true;
              ...
              if (quad) {
                if (minmin.uv[0] < 0.01 && minmin.uv[1] < 0.01) {
                  ...
                  ctx.fillStyle = `rgba(${(v0.col[0] >> 0) & 0xff}, ${
                    (v0.col[0] >> 8) & 0xff
                  }, ${(v0.col[0] >> 16) & 0xff}, ${
                    ((v0.col[0] >> 24) & 0xff) / 0xff
                  })`;
                  ctx.fill();
                } else {
                  // no vertex color
                  ...
                  ctx.drawImage(
                    image,
                    minmin.uv[0] * image.width,
                    minmin.uv[1] * image.height,
                    (maxmax.uv[0] - minmin.uv[0]) * image.width,
                    (maxmax.uv[1] - minmin.uv[1]) * image.height,
                    minmin.pos[0],
                    minmin.pos[1],
                    maxmax.pos[0] - minmin.pos[0],
                    maxmax.pos[1] - minmin.pos[1]
...
export function CreateFontsTexture(): void {
  ...
  // Backup GL state
  const last_texture: WebGLTexture | null =
    gl && gl.getParameter(gl.TEXTURE_BINDING_2D);
...
export function CreateDeviceObjects(): void {
  ...
  gl &&
    gl.shaderSource(g_FragHandle as WebGLShader, fragment_shader.join("\n"));
  gl && gl.compileShader(g_VertHandle as WebGLShader);
  gl && gl.compileShader(g_FragHandle as WebGLShader);
  gl &&
    gl.attachShader(
      g_ShaderHandle as WebGLProgram,
      g_VertHandle as WebGLShader
    );
  gl &&
    gl.attachShader(
      g_ShaderHandle as WebGLProgram,
      g_FragHandle as WebGLShader
    );
  gl && gl.linkProgram(g_ShaderHandle as WebGLProgram);

  g_AttribLocationTex =
    gl && gl.getUniformLocation(g_ShaderHandle as WebGLProgram, "Texture");
  g_AttribLocationProjMtx =
    gl && gl.getUniformLocation(g_ShaderHandle as WebGLProgram, "ProjMtx");
  g_AttribLocationPosition =
    (gl && gl.getAttribLocation(g_ShaderHandle as WebGLProgram, "Position")) ||
    0;
  g_AttribLocationUV =
    (gl && gl.getAttribLocation(g_ShaderHandle as WebGLProgram, "UV")) || 0;
  g_AttribLocationColor =
    (gl && gl.getAttribLocation(g_ShaderHandle as WebGLProgram, "Color")) || 0;
