(function (global, factory) {
    ...
    function ShowDemoWindow(p_open = null) {
        ...
        ImGui.SetNextWindowSize(new ImGui.ImVec2(550, 680), ImGui.ImGuiCond.FirstUseEver);
        ...
        ImGui.Text(`dear imgui says hello. (${ImGui.IMGUI_VERSION})`);
        ImGui.Spacing();
    ...
    function ShowDemoWindowWidgets() {
        ...
        if (ImGui.TreeNode("Basic")) {
            ...
            for (let i = 0; i < 7; i++) {
                if (i > 0)
                    ImGui.SameLine();
                ImGui.PushID(i);
                ImGui.PushStyleColor(ImGui.ImGuiCol.Button, ImGui.ImColor.HSV(i / 7.0, 0.6, 0.6));
                ImGui.PushStyleColor(ImGui.ImGuiCol.ButtonHovered, ImGui.ImColor.HSV(i / 7.0, 0.7, 0.7));
                ImGui.PushStyleColor(ImGui.ImGuiCol.ButtonActive, ImGui.ImColor.HSV(i / 7.0, 0.8, 0.8));
                ImGui.Button("Click");
            ...
            if (ImGui.ArrowButton("##left", ImGui.ImGuiDir.Left)) {
                counter.value--;
            }
            ImGui.SameLine(0.0, spacing);
            if (ImGui.ArrowButton("##right", ImGui.ImGuiDir.Right)) {
                counter.value++;
            }
            ...
            if (ImGui.IsItemHovered()) {
                ...
                ImGui.PlotLines("Curve", arr.value, ImGui.IM_ARRAYSIZE(arr.value));
                ImGui.EndTooltip();
            }
            ImGui.Separator();
            ImGui.LabelText("label", "Value");
            {
                ...
                ImGui.Combo("combo", (value = item_current.value) => item_current.value = value, items, ImGui.IM_ARRAYSIZE(items));
                ImGui.SameLine();
            ...
            {
                /* static */ const str0 = STATIC("str0", new ImGui.ImStringBuffer(128, "Hello, world!"));
                ImGui.InputText("input text", str0.value, ImGui.IM_ARRAYSIZE(str0.value));
                ImGui.SameLine();
                HelpMarker("USER:\nHold SHIFT or use mouse to select text.\n" + "CTRL+Left/Right to word jump.\n" + "CTRL+A or double-click to select all.\n" + "CTRL+X,CTRL+C,CTRL+V clipboard.\n" + "CTRL+Z,CTRL+Y undo/redo.\n" + "ESCAPE to revert.\n\nPROGRAMMER:\nYou can use the ImGuiInputTextFlags_CallbackResize facility if you need to wire InputText() to a dynamic string type. See misc/cpp/imgui_stdlib.h for an example (this is not demonstrated in imgui_demo.cpp).");
                /* static */ const str1 = STATIC("str1", new ImGui.ImStringBuffer(128, ""));
                ImGui.InputTextWithHint("input text (w/ hint)", "enter text here", str1.value, ImGui.IM_ARRAYSIZE(str1.value));
            ...
            }
            {
                ...
                ImGui.ListBox("listbox\n(single select)", (value = listbox_item_current.value) => listbox_item_current.value = value, listbox_items, ImGui.IM_ARRAYSIZE(listbox_items), 4);
        ...
        if (ImGui.TreeNode("Trees")) {
            if (ImGui.TreeNode("Basic trees")) {
                for (let i = 0; i < 5; i++) {
                    ...
                    if (i == 0)
                        ImGui.SetNextItemOpen(true, ImGui.ImGuiCond.Once);
            ...
            if (ImGui.TreeNode("Advanced, with Selectable nodes")) {
                ...
                ImGui.PushStyleVar(ImGui.ImGuiStyleVar.IndentSpacing, ImGui.GetFontSize() * 3); // Increase spacing to differentiate leaves from expanded contents.
                for (let i = 0; i < 6; i++) {
                    // Disable the default open on single-click behavior and pass in Selected flag according to our selection state.
                    let node_flags = ImGui.ImGuiTreeNodeFlags.OpenOnArrow | ImGui.ImGuiTreeNodeFlags.OpenOnDoubleClick;
                    ...
                    if (i < 3) {
                        ...
                        node_flags |= ImGui.ImGuiTreeNodeFlags.Leaf | ImGui.ImGuiTreeNodeFlags.NoTreePushOnOpen; // ImGuiTreeNodeFlags.Bullet
        ...
        if (ImGui.TreeNode("Text")) {
            if (ImGui.TreeNode("Colored Text")) {
                ...
                ImGui.TextColored(new ImGui.ImVec4(1.0, 0.0, 1.0, 1.0), "Pink");
                ImGui.TextColored(new ImGui.ImVec4(1.0, 1.0, 0.0, 1.0), "Yellow");
            ...
            if (ImGui.TreeNode("Word Wrapping")) {
                ...
                ImGui.GetWindowDrawList().AddRectFilled(new ImGui.ImVec2(pos.x + wrap_width.value, pos.y), new ImGui.ImVec2(pos.x + wrap_width.value + 10, pos.y + ImGui.GetTextLineHeight()), ImGui.IM_COL32(255, 0, 255, 255));
                ...
                ImGui.GetWindowDrawList().AddRect(ImGui.GetItemRectMin(), ImGui.GetItemRectMax(), ImGui.IM_COL32(255, 255, 0, 255));
                ImGui.PopTextWrapPos();
            ...
            if (ImGui.TreeNode("UTF-8 Text")) {
                ...
                /* static */ const buf = STATIC("buf", new ImGui.ImStringBuffer(32, "日本語"));
                //static char buf[32] = u8"NIHONGO"; // <- this is how you would write it with C++11, using real kanjis
                ImGui.InputText("UTF-8 input", buf.value, ImGui.IM_ARRAYSIZE(buf.value));
                ImGui.TreePop();
            }
            ImGui.TreePop();
        }
        if (ImGui.TreeNode("Images")) {
            ...
            ImGui.Image(my_tex_id, new ImGui.ImVec2(my_tex_w, my_tex_h), new ImGui.ImVec2(0, 0), new ImGui.ImVec2(1, 1), new ImGui.ImVec4(1.0, 1.0, 1.0, 1.0), new ImGui.ImVec4(1.0, 1.0, 1.0, 0.5));
            if (ImGui.IsItemHovered()) {
                ImGui.BeginTooltip();
                ...
                const uv0 = new ImGui.ImVec2((region_x) / my_tex_w, (region_y) / my_tex_h);
                const uv1 = new ImGui.ImVec2((region_x + region_sz) / my_tex_w, (region_y + region_sz) / my_tex_h);
                ImGui.Image(my_tex_id, new ImGui.ImVec2(region_sz * zoom, region_sz * zoom), uv0, uv1, new ImGui.ImColor(255, 255, 255, 255).toImVec4(), new ImGui.ImColor(255, 255, 255, 128).toImVec4());
                ImGui.EndTooltip();
            }
            ...
            for (let i = 0; i < 8; i++) {
                ...
                if (ImGui.ImageButton(my_tex_id, new ImGui.ImVec2(32, 32), new ImGui.ImVec2(0, 0), new ImGui.ImVec2(32.0 / my_tex_w, 32 / my_tex_h), frame_padding, new ImGui.ImVec4(0, 0, 0, 1)))
        ...
        if (ImGui.TreeNode("Combo")) {
            ...
            ImGui.Combo("combo 3 (array)", (value = item_current_3.value) => item_current_3.value = value, items, ImGui.IM_ARRAYSIZE(items));
            ...
            ImGui.Combo("combo 4 (function)", (value = item_current_4.value) => item_current_4.value = value, FuncHolder.ItemGetter, items, ImGui.IM_ARRAYSIZE(items));
            ImGui.TreePop();
        }
        if (ImGui.TreeNode("Selectables")) {
            ...
            if (ImGui.TreeNode("Basic")) {
                ...
                if (ImGui.Selectable("5. I am double clickable", selection.value[4], ImGui.ImGuiSelectableFlags.AllowDoubleClick))
            ...
            if (ImGui.TreeNode("Grid")) {
                ...
                for (let i = 0; i < 4 * 4; i++) {
                    ImGui.PushID(i);
                    if (ImGui.Selectable("Sailor", (value = selected.value[i]) => selected.value[i] = value, 0, new ImGui.ImVec2(50, 50))) {
            ...
            if (ImGui.TreeNode("Alignment")) {
                ...
                for (let y = 0; y < 3; y++) {
                    for (let x = 0; x < 3; x++) {
                        const alignment = new ImGui.ImVec2(x / 2.0, y / 2.0);
                        // char name[32];
                        // sprintf(name, "(%.1f,%.1f)", alignment.x, alignment.y);
                        const name = `(${alignment.x.toFixed(1)},${alignment.y.toFixed(1)})`;
                        if (x > 0)
                            ImGui.SameLine();
                        ImGui.PushStyleVar(ImGui.ImGuiStyleVar.SelectableTextAlign, alignment);
                        // ImGui.Selectable(name, &selected[3*y+x], ImGuiSelectableFlags_None, ImVec2(80,80));
                        ImGui.Selectable(name, (value = selected.value[3 * y + x]) => selected.value[3 * y + x] = value, ImGui.ImGuiSelectableFlags.None, new ImGui.ImVec2(80, 80));
                        ImGui.PopStyleVar();
                    }
                }
                ImGui.TreePop();
        ...
        if (ImGui.TreeNode("Text Input")) {
            if (ImGui.TreeNode("Multi-line Text Input")) {
                ...
                /* static */ const text = STATIC("text", new ImGui.ImStringBuffer(1024 * 16, "/*\n" +
                    ...
                    "label:\n" +
                    "\tlock cmpxchg8b eax\n"));
                /* static */ const flags = STATIC("flags", ImGui.ImGuiInputTextFlags.AllowTabInput);
                ...
                ImGui.CheckboxFlags("ImGuiInputTextFlags_ReadOnly", (value = flags.value) => flags.value = value, ImGui.ImGuiInputTextFlags.ReadOnly);
                ImGui.CheckboxFlags("ImGuiInputTextFlags_AllowTabInput", (value = flags.value) => flags.value = value, ImGui.ImGuiInputTextFlags.AllowTabInput);
                ImGui.CheckboxFlags("ImGuiInputTextFlags_CtrlEnterForNewLine", (value = flags.value) => flags.value = value, ImGui.ImGuiInputTextFlags.CtrlEnterForNewLine);
                ImGui.InputTextMultiline("##source", text.value, ImGui.IM_ARRAYSIZE(text.value), new ImGui.ImVec2(-1.0, ImGui.GetTextLineHeight() * 16), flags.value);
                ImGui.TreePop();
            }
            if (ImGui.TreeNode("Filtered Text Input")) {
                /* static */ const buf1 = STATIC("buf1", new ImGui.ImStringBuffer(64, ""));
                ImGui.InputText("default", buf1.value, ImGui.IM_ARRAYSIZE(buf1.value));
                /* static */ const buf2 = STATIC("buf2", new ImGui.ImStringBuffer(64, ""));
                ImGui.InputText("decimal", buf2.value, ImGui.IM_ARRAYSIZE(buf2.value), ImGui.ImGuiInputTextFlags.CharsDecimal);
                /* static */ const buf3 = STATIC("buf3", new ImGui.ImStringBuffer(64, ""));
                ImGui.InputText("hexadecimal", buf3.value, ImGui.IM_ARRAYSIZE(buf3.value), ImGui.ImGuiInputTextFlags.CharsHexadecimal | ImGui.ImGuiInputTextFlags.CharsUppercase);
                /* static */ const buf4 = STATIC("buf4", new ImGui.ImStringBuffer(64, ""));
                ImGui.InputText("uppercase", buf4.value, ImGui.IM_ARRAYSIZE(buf4.value), ImGui.ImGuiInputTextFlags.CharsUppercase);
                /* static */ const buf5 = STATIC("buf5", new ImGui.ImStringBuffer(64, ""));
                ImGui.InputText("no blank", buf5.value, ImGui.IM_ARRAYSIZE(buf5.value), ImGui.ImGuiInputTextFlags.CharsNoBlank);
                class TextFilters {
                    static FilterImGuiLetters(data) { if (data.EventChar < 256 && /[imgui]/.test(String.fromCharCode(data.EventChar)))
                        return 0; return 1; }
                }
                /* static */ const buf6 = STATIC("buf6", new ImGui.ImStringBuffer(64, ""));
                ImGui.InputText("\"imgui\" letters", buf6.value, ImGui.IM_ARRAYSIZE(buf6.value), ImGui.ImGuiInputTextFlags.CallbackCharFilter, TextFilters.FilterImGuiLetters);
                ImGui.Text("Password input");
                /* static */ const bufpass = STATIC("bufpass", new ImGui.ImStringBuffer(64, "password123"));
                ImGui.InputText("password", bufpass.value, ImGui.IM_ARRAYSIZE(bufpass.value), ImGui.ImGuiInputTextFlags.Password | ImGui.ImGuiInputTextFlags.CharsNoBlank);
                ImGui.SameLine();
                HelpMarker("Display all characters as '*'.\nDisable clipboard cut and copy.\nDisable logging.\n");
                ImGui.InputText("password (clear)", bufpass.value, ImGui.IM_ARRAYSIZE(bufpass.value), ImGui.ImGuiInputTextFlags.CharsNoBlank);
                ImGui.TreePop();
            }
        ...
        if (ImGui.TreeNode("Plots Widgets")) {
            ...
            ImGui.PlotLines("Frame Times", arr.value, ImGui.IM_ARRAYSIZE(arr.value));
            ...
            while (refresh_time.value < ImGui.GetTime()) // Create dummy data at fixed 60 hz rate for the demo
             {
                ...
                values_offset.value = (values_offset.value + 1) % ImGui.IM_ARRAYSIZE(values.value);
                phase.value += 0.10 * values_offset.value;
                refresh_time.value += 1.0 / 60.0;
            }
            ImGui.PlotLines("Lines", values.value, ImGui.IM_ARRAYSIZE(values.value), values_offset.value, "avg 0.0", -1.0, 1.0, new ImGui.ImVec2(0, 80));
            ImGui.PlotHistogram("Histogram", arr.value, ImGui.IM_ARRAYSIZE(arr.value), 0, null, 0.0, 1.0, new ImGui.ImVec2(0, 80));
            ...
            ImGui.PlotLines("Lines", func, null, display_count.value, 0, null, -1.0, 1.0, new ImGui.ImVec2(0, 80));
            ImGui.PlotHistogram("Histogram", func, null, display_count.value, 0, null, -1.0, 1.0, new ImGui.ImVec2(0, 80));
            ImGui.Separator();
            ...
            ImGui.ProgressBar(progress.value, new ImGui.ImVec2(0.0, 0.0));
            ...
            ImGui.ProgressBar(progress.value, new ImGui.ImVec2(0., 0.), buf);
            ImGui.TreePop();
        }
        if (ImGui.TreeNode("Color/Picker Widgets")) {
            /* static */ const color = STATIC("color#863", new ImGui.ImVec4(114.0 / 255.0, 144.0 / 255.0, 154.0 / 255.0, 200.0 / 255.0));
            ...
            const misc_flags = (hdr.value ? ImGui.ImGuiColorEditFlags.HDR : 0) | (drag_and_drop.value ? 0 : ImGui.ImGuiColorEditFlags.NoDragDrop) | (alpha_half_preview.value ? ImGui.ImGuiColorEditFlags.AlphaPreviewHalf : (alpha_preview.value ? ImGui.ImGuiColorEditFlags.AlphaPreview : 0)) | (options_menu.value ? 0 : ImGui.ImGuiColorEditFlags.NoOptions);
            ImGui.Text("Color widget:");
            ImGui.SameLine();
            HelpMarker("Click on the colored square to open a color picker.\nCTRL+click on individual component to input value.\n");
            ImGui.ColorEdit3("MyColor##1", color.value, misc_flags);
            ImGui.Text("Color widget HSV with Alpha:");
            ImGui.ColorEdit4("MyColor##2", color.value, ImGui.ImGuiColorEditFlags.DisplayHSV | misc_flags);
            ImGui.Text("Color widget with Float Display:");
            ImGui.ColorEdit4("MyColor##2f", color.value, ImGui.ImGuiColorEditFlags.Float | misc_flags);
            ...
            ImGui.ColorEdit4("MyColor##3", color.value, ImGui.ImGuiColorEditFlags.NoInputs | ImGui.ImGuiColorEditFlags.NoLabel | misc_flags);
            ...
            if (saved_palette_init.value) {
                for (let n = 0; n < 32; n++) {
                    saved_palette.value[n] = new ImGui.ImVec4();
            ...
            /* static */ const backup_color = STATIC("backup_color", new ImGui.ImVec4());
            ...
            if (ImGui.BeginPopup("mypicker")) {
                ...
                ImGui.Separator();
                ImGui.ColorPicker4("##picker", color.value, misc_flags | ImGui.ImGuiColorEditFlags.NoSidePreview | ImGui.ImGuiColorEditFlags.NoSmallPreview);
                ImGui.SameLine();
                ImGui.BeginGroup(); // Lock X position
                ImGui.Text("Current");
                ImGui.ColorButton("##current", color.value, ImGui.ImGuiColorEditFlags.NoPicker | ImGui.ImGuiColorEditFlags.AlphaPreviewHalf, new ImGui.ImVec2(60, 40));
                ImGui.Text("Previous");
                if (ImGui.ColorButton("##previous", backup_color.value, ImGui.ImGuiColorEditFlags.NoPicker | ImGui.ImGuiColorEditFlags.AlphaPreviewHalf, new ImGui.ImVec2(60, 40)))
                    color.value.Copy(backup_color.value);
                ImGui.Separator();
                ImGui.Text("Palette");
                for (let n = 0; n < ImGui.IM_ARRAYSIZE(saved_palette.value); n++) {
                    ImGui.PushID(n);
                    if ((n % 8) !== 0)
                        ImGui.SameLine(0.0, ImGui.GetStyle().ItemSpacing.y);
                    if (ImGui.ColorButton("##palette", saved_palette.value[n], ImGui.ImGuiColorEditFlags.NoAlpha | ImGui.ImGuiColorEditFlags.NoPicker | ImGui.ImGuiColorEditFlags.NoTooltip, new ImGui.ImVec2(20, 20)))
                        color.value.Copy(new ImGui.ImVec4(saved_palette.value[n].x, saved_palette.value[n].y, saved_palette.value[n].z, color.value.w)); // Preserve alpha!
            ...
            ImGui.ColorButton("MyColor##3c", color.value, misc_flags, new ImGui.ImVec2(80, 80));
            ...
            /* static */ const ref_color_v = STATIC("ref_color_v", new ImGui.ImVec4(1.0, 0.0, 1.0, 0.5));
            ...
            if (side_preview) {
                ...
                if (ref_color.value) {
                    ImGui.SameLine();
                    ImGui.ColorEdit4("##RefColor", ref_color_v.value, ImGui.ImGuiColorEditFlags.NoInputs | misc_flags);
                }
            }
            ...
            if (!alpha.value)
                flags |= ImGui.ImGuiColorEditFlags.NoAlpha; // This is by default if you call ColorPicker3() instead of ColorPicker4()
            if (alpha_bar.value)
            ...
            if (display_mode.value === 1)
                flags |= ImGui.ImGuiColorEditFlags.NoInputs; // Disable all RGB/HSV/Hex displays
            if (display_mode.value === 2)
                flags |= ImGui.ImGuiColorEditFlags.DisplayRGB; // Override display mode
            ...
            if (ImGui.Button("Default: Uint8 + HSV + Hue Bar"))
                ImGui.SetColorEditOptions(ImGui.ImGuiColorEditFlags.Uint8 | ImGui.ImGuiColorEditFlags.DisplayHSV | ImGui.ImGuiColorEditFlags.PickerHueBar);
            if (ImGui.Button("Default: Float + HDR + Hue Wheel"))
                ImGui.SetColorEditOptions(ImGui.ImGuiColorEditFlags.Float | ImGui.ImGuiColorEditFlags.HDR | ImGui.ImGuiColorEditFlags.PickerHueWheel);
            // HSV encoded support (to avoid RGB<>HSV round trips and singularities when S==0 or V==0)
            /* static */ const color_stored_as_hsv = STATIC("color_stored_as_hsv", new ImGui.ImVec4(0.23, 1.0, 1.0, 1.0));
            ImGui.Spacing();
            ...
            ImGui.ColorEdit4("HSV shown as RGB##1", color_stored_as_hsv.value, ImGui.ImGuiColorEditFlags.DisplayRGB | ImGui.ImGuiColorEditFlags.InputHSV | ImGui.ImGuiColorEditFlags.Float);
            ImGui.ColorEdit4("HSV shown as HSV##1", color_stored_as_hsv.value, ImGui.ImGuiColorEditFlags.DisplayHSV | ImGui.ImGuiColorEditFlags.InputHSV | ImGui.ImGuiColorEditFlags.Float);
        ...
        if (ImGui.TreeNode("Data Types")) {
            ...
            ImGui.InputScalar("input s8 hex", s8_v.value, inputs_step.value ? s8_one : null, null, "%08X", ImGui.ImGuiInputTextFlags.CharsHexadecimal);
            ImGui.InputScalar("input s16", s16_v.value, inputs_step.value ? s16_one : null, null, "%d");
            ImGui.InputScalar("input s16 hex", s16_v.value, inputs_step.value ? s16_one : null, null, "%08X", ImGui.ImGuiInputTextFlags.CharsHexadecimal);
            ImGui.InputScalar("input s32", s32_v.value, inputs_step.value ? s32_one : null, null, "%d");
            ImGui.InputScalar("input s32 hex", s32_v.value, inputs_step.value ? s32_one : null, null, "%08X", ImGui.ImGuiInputTextFlags.CharsHexadecimal);
            ImGui.InputScalar("input u32", u32_v.value, inputs_step.value ? u32_one : null, null, "%u");
            ImGui.InputScalar("input u32 hex", u32_v.value, inputs_step.value ? u32_one : null, null, "%08X", ImGui.ImGuiInputTextFlags.CharsHexadecimal);
        ...
        if (ImGui.TreeNode("Vertical Sliders")) {
            const spacing = 4;
            ImGui.PushStyleVar(ImGui.ImGuiStyleVar.ItemSpacing, new ImGui.ImVec2(spacing, spacing));
            /* static */ const int_value = STATIC("int_value", 0);
            ImGui.VSliderInt("##int", new ImGui.ImVec2(18, 160), (value = int_value.value) => int_value.value = value, 0, 5);
            ImGui.SameLine();
            ...
            for (let i = 0; i < 7; i++) {
                if (i > 0)
                    ImGui.SameLine();
                ImGui.PushID(i);
                ImGui.PushStyleColor(ImGui.ImGuiCol.FrameBg, ImGui.ImColor.HSV(i / 7.0, 0.5, 0.5));
                ImGui.PushStyleColor(ImGui.ImGuiCol.FrameBgHovered, ImGui.ImColor.HSV(i / 7.0, 0.6, 0.5));
                ImGui.PushStyleColor(ImGui.ImGuiCol.FrameBgActive, ImGui.ImColor.HSV(i / 7.0, 0.7, 0.5));
                ImGui.PushStyleColor(ImGui.ImGuiCol.SliderGrab, ImGui.ImColor.HSV(i / 7.0, 0.9, 0.9));
                ImGui.VSliderFloat("##v", new ImGui.ImVec2(18, 160), (value = values.value[i]) => values.value[i] = value, 0.0, 1.0, "");
            ...
            const rows = 3;
            const small_slider_size = new ImGui.ImVec2(18, (160.0 - (rows - 1) * spacing) / rows);
            ...
            for (let i = 0; i < 4; i++) {
                if (i > 0)
                    ImGui.SameLine();
                ImGui.PushID(i);
                ImGui.PushStyleVar(ImGui.ImGuiStyleVar.GrabMinSize, 40);
                ImGui.VSliderFloat("##v", new ImGui.ImVec2(40, 160), (value = values.value[i]) => values.value[i] = value, 0.0, 1.0, "%.2f\nsec");
                ImGui.PopStyleVar();
                ImGui.PopID();
            }
            ImGui.PopID();
        ...
        if (ImGui.TreeNode("Drag and Drop")) {
            ...
            }
            {
                ...
                for (let n = 0; n < ImGui.IM_ARRAYSIZE(names.value); n++) {
                    ImGui.PushID(n);
                    if ((n % 3) != 0)
                        ImGui.SameLine();
                    ImGui.Button(names.value[n], new ImGui.ImVec2(60, 60));
        ...
        if (ImGui.TreeNode("Querying Status (Active/Focused/Hovered etc.)")) {
            ...
            if (item_type.value === 9) {
                ...
                ret = ImGui.ListBox("ITEM: ListBox", (value = current.value) => current.value = value, items, ImGui.IM_ARRAYSIZE(items), ImGui.IM_ARRAYSIZE(items));
            }
            ImGui.BulletText(`Return value = ${ret}\n` +
                `IsItemFocused() = ${ImGui.IsItemFocused()}\n` +
                `IsItemHovered() = ${ImGui.IsItemHovered()}\n` +
                `IsItemHovered(_AllowWhenBlockedByPopup) = ${ImGui.IsItemHovered(ImGui.ImGuiHoveredFlags.AllowWhenBlockedByPopup)}\n` +
                `IsItemHovered(_AllowWhenBlockedByActiveItem) = ${ImGui.IsItemHovered(ImGui.ImGuiHoveredFlags.AllowWhenBlockedByActiveItem)}\n` +
                `IsItemHovered(_AllowWhenOverlapped) = ${ImGui.IsItemHovered(ImGui.ImGuiHoveredFlags.AllowWhenOverlapped)}\n` +
                `IsItemhovered(_RectOnly) = ${ImGui.IsItemHovered(ImGui.ImGuiHoveredFlags.RectOnly)}\n` +
            ...
            if (embed_all_inside_a_child_window.value)
                ImGui.BeginChild("outer_child", new ImGui.ImVec2(0, ImGui.GetFontSize() * 20), true);
            // Testing IsWindowFocused() function with its various flags. Note that the flags can be combined.
            ImGui.BulletText(`IsWindowFocused() = ${ImGui.IsWindowFocused()}\n` +
                `IsWindowFocused(_ChildWindows) = ${ImGui.IsWindowFocused(ImGui.ImGuiFocusedFlags.ChildWindows)}\n` +
                `IsWindowFocused(_ChildWindows|_RootWindow) = ${ImGui.IsWindowFocused(ImGui.ImGuiFocusedFlags.ChildWindows | ImGui.ImGuiFocusedFlags.RootWindow)}\n` +
                `IsWindowFocused(_RootWindow) = ${ImGui.IsWindowFocused(ImGui.ImGuiFocusedFlags.RootWindow)}\n` +
                `IsWindowFocused(_AnyWindow) = ${ImGui.IsWindowFocused(ImGui.ImGuiFocusedFlags.AnyWindow)}\n`);
            // Testing IsWindowHovered() function with its various flags. Note that the flags can be combined.
            ImGui.BulletText(`IsWindowHovered() = ${ImGui.IsWindowHovered()}\n` +
                `IsWindowHovered(_AllowWhenBlockedByPopup) = ${ImGui.IsWindowHovered(ImGui.ImGuiHoveredFlags.AllowWhenBlockedByPopup)}\n` +
                `IsWindowHovered(_AllowWhenBlockedByActiveItem) = ${ImGui.IsWindowHovered(ImGui.ImGuiHoveredFlags.AllowWhenBlockedByActiveItem)}\n` +
                `IsWindowHovered(_ChildWindows) = ${ImGui.IsWindowHovered(ImGui.ImGuiHoveredFlags.ChildWindows)}\n` +
                `IsWindowHovered(_ChildWindows|_RootWindow) = ${ImGui.IsWindowHovered(ImGui.ImGuiHoveredFlags.ChildWindows | ImGui.ImGuiHoveredFlags.RootWindow)}\n` +
                `IsWindowFocused(_ChildWindows|_AllowWhenBlockedByPopup) = ${ImGui.IsWindowFocused(ImGui.ImGuiHoveredFlags.ChildWindows | ImGui.ImGuiHoveredFlags.AllowWhenBlockedByPopup)}\n` +
                `IsWindowHovered(_RootWindow) = ${ImGui.IsWindowHovered(ImGui.ImGuiHoveredFlags.RootWindow)}\n` +
                `IsWindowHovered(_AnyWindow) = ${ImGui.IsWindowHovered(ImGui.ImGuiHoveredFlags.AnyWindow)}\n`);
            ImGui.BeginChild("child", new ImGui.ImVec2(0, 50), true);
    ...
    function ShowDemoWindowLayout() {
        ...
        if (ImGui.TreeNode("Child windows")) {
            ...
            ImGui.SameLine();
            ImGui.SetNextItemWidth(100);
            goto_line = ImGui.InputInt("##Line", (value = line.value) => line.value = value, 0, 0, ImGui.ImGuiInputTextFlags.EnterReturnsTrue) || goto_line;
            // Child 1: no border, enable horizontal scrollbar
            {
                const window_flags = ImGui.ImGuiWindowFlags.HorizontalScrollbar | (disable_mouse_wheel.value ? ImGui.ImGuiWindowFlags.NoScrollWithMouse : 0);
                ImGui.BeginChild("Child1", new ImGui.ImVec2(ImGui.GetWindowContentRegionWidth() * 0.5, 260), false, window_flags);
                for (let i = 0; i < 100; i++) {
            ...
            {
                const window_flags = (disable_mouse_wheel.value ? ImGui.ImGuiWindowFlags.NoScrollWithMouse : 0) | (disable_menu.value ? 0 : ImGui.ImGuiWindowFlags.MenuBar);
                ImGui.PushStyleVar(ImGui.ImGuiStyleVar.ChildRounding, 5.0);
                ImGui.BeginChild("Child2", new ImGui.ImVec2(0, 260), true, window_flags);
                ...
                for (let i = 0; i < 100; i++) {
                    ...
                    ImGui.Button(buf, new ImGui.ImVec2(-1.0, 0.0));
                    ImGui.NextColumn();
                }
                ImGui.EndChild();
                ImGui.PopStyleVar();
            ...
            //   See "Widgets" -> "Querying Status (Active/Focused/Hovered etc.)" section for more details about this.
            {
                ...
                ImGui.PushStyleColor(ImGui.ImGuiCol.ChildBg, ImGui.IM_COL32(255, 0, 0, 100));
                ImGui.BeginChild("blah", new ImGui.ImVec2(200, 100), true, ImGui.ImGuiWindowFlags.None);
                for (let n = 0; n < 50; n++)
        ...
        if (ImGui.TreeNode("Basic Horizontal Layout")) {
            ...
            ImGui.SameLine();
            ImGui.TextColored(new ImGui.ImVec4(1, 1, 0, 1), "Sailor");
            // Adjust spacing
            ...
            ImGui.Combo("Combo", (value = item.value) => item.value = value, items, ImGui.IM_ARRAYSIZE(items));
            ImGui.SameLine();
            ...
            for (let i = 0; i < 4; i++) {
                if (i > 0)
                    ImGui.SameLine();
                ImGui.PushID(i);
                ImGui.ListBox("", (value = selection.value[i]) => selection.value[i] = value, items, ImGui.IM_ARRAYSIZE(items));
                ImGui.PopID();
                if (ImGui.IsItemHovered())
            ...
            ImGui.PopItemWidth();
            // Dummy
            const button_sz = new ImGui.ImVec2(40, 40);
        ...
        if (ImGui.TreeNode("Tabs")) {
            if (ImGui.TreeNode("Basic")) {
                const tab_bar_flags = ImGui.ImGuiTabBarFlags.None;
            ...
            if (ImGui.TreeNode("Advanced & Close Button")) {
                ...
                /* static */ const tab_bar_flags = STATIC("tab_bar_flags", ImGui.ImGuiTabBarFlags.Reorderable);
                ...
                ImGui.CheckboxFlags("ImGuiTabBarFlags_TabListPopupButton", (value = tab_bar_flags.value) => tab_bar_flags.value = value, ImGui.ImGuiTabBarFlags.TabListPopupButton);
                ...
                if ((tab_bar_flags.value & ImGui.ImGuiTabBarFlags.FittingPolicyMask_) === 0)
                    tab_bar_flags.value |= ImGui.ImGuiTabBarFlags.FittingPolicyDefault_;
                if (ImGui.CheckboxFlags("ImGuiTabBarFlags_FittingPolicyResizeDown", (value = tab_bar_flags.value) => tab_bar_flags.value = value, ImGui.ImGuiTabBarFlags.FittingPolicyResizeDown))
                    tab_bar_flags.value &= ~(ImGui.ImGuiTabBarFlags.FittingPolicyMask_ ^ ImGui.ImGuiTabBarFlags.FittingPolicyResizeDown);
                if (ImGui.CheckboxFlags("ImGuiTabBarFlags_FittingPolicyScroll", (value = tab_bar_flags.value) => tab_bar_flags.value = value, ImGui.ImGuiTabBarFlags.FittingPolicyScroll))
                    tab_bar_flags.value &= ~(ImGui.ImGuiTabBarFlags.FittingPolicyMask_ ^ ImGui.ImGuiTabBarFlags.FittingPolicyScroll);
                // Tab Bar
                const names = ["Artichoke", "Beetroot", "Celery", "Daikon"];
                /* static */ const opened = STATIC("opened", [true, true, true, true]); // Persistent user state
                for (let n = 0; n < ImGui.IM_ARRAYSIZE(opened.value); n++) {
                    if (n > 0) {
                        ImGui.SameLine();
                    }
        ...
        if (ImGui.TreeNode("Groups")) {
            ...
            ImGui.PlotHistogram("##values", values, ImGui.IM_ARRAYSIZE(values), 0, null, 0.0, 1.0, size);
            ImGui.Button("ACTION", new ImGui.ImVec2((size.x - ImGui.GetStyle().ItemSpacing.x) * 0.5, size.y));
            ImGui.SameLine();
            ImGui.Button("REACTION", new ImGui.ImVec2((size.x - ImGui.GetStyle().ItemSpacing.x) * 0.5, size.y));
            ImGui.EndGroup();
            ImGui.SameLine();
        ...
        if (ImGui.TreeNode("Scrolling")) {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                const child_flags = ImGui.ImGuiWindowFlags.MenuBar;
                ImGui.BeginChild(ImGui.GetID(i), new ImGui.ImVec2(child_w, 200.0), true, child_flags);
                if (scroll_to_off)
                ...
                for (let line = 0; line < 100; line++) {
                    if (track.value && line === track_line.value) {
                        ImGui.TextColored(new ImGui.ImVec4(1, 1, 0, 1), `Line ${line}`);
        ...
        if (ImGui.TreeNode("Horizontal Scrolling")) {
            ...
            ImGui.PushStyleVar(ImGui.ImGuiStyleVar.FramePadding, new ImGui.ImVec2(2.0, 1.0));
            ImGui.BeginChild("scrolling", new ImGui.ImVec2(0, ImGui.GetFrameHeightWithSpacing() * 7 + 30), true, ImGui.ImGuiWindowFlags.HorizontalScrollbar);
            for (let line = 0; line < lines.value; line++) {
                ...
                for (let n = 0; n < num_buttons; n++) {
                    ...
                    const hue = n * 0.05;
                    ImGui.PushStyleColor(ImGui.ImGuiCol.Button, ImGui.ImColor.HSV(hue, 0.6, 0.6));
                    ImGui.PushStyleColor(ImGui.ImGuiCol.ButtonHovered, ImGui.ImColor.HSV(hue, 0.7, 0.7));
                    ImGui.PushStyleColor(ImGui.ImGuiCol.ButtonActive, ImGui.ImColor.HSV(hue, 0.8, 0.8));
                    ImGui.Button(label, new ImGui.ImVec2(40.0 + Math.sin(line + n) * 20.0, 0.0));
                    ImGui.PopStyleColor(3);
                    ImGui.PopID();
                }
            }
        ...
        if (ImGui.TreeNode("Clipping")) {
            /* static */ const size = STATIC("size", new ImGui.ImVec2(100, 100)), offset = STATIC("offset", new ImGui.ImVec2(50, 20));
            ImGui.TextWrapped("On a per-widget basis we are occasionally clipping text CPU-side if it won't fit in its frame. Otherwise we are doing coarser clipping + passing a scissor rectangle to the renderer. The system is designed to try minimizing both execution and CPU/GPU rendering cost.");
            ImGui.DragFloat2("size", size.value, 0.5, 1.0, 200.0, "%.0f");
            ImGui.TextWrapped("(Click and drag)");
            const pos = ImGui.GetCursorScreenPos();
            const clip_rect = new ImGui.ImVec4(pos.x, pos.y, pos.x + size.value.x, pos.y + size.value.y);
            ...
            ImGui.GetWindowDrawList().AddRectFilled(pos, new ImGui.ImVec2(pos.x + size.value.x, pos.y + size.value.y), ImGui.IM_COL32(90, 90, 120, 255));
            ImGui.GetWindowDrawList().AddText(ImGui.GetFont(), ImGui.GetFontSize() * 2.0, new ImGui.ImVec2(pos.x + offset.value.x, pos.y + offset.value.y), ImGui.IM_COL32(255, 255, 255, 255), "Line 1 hello\nLine 2 clip me!", null, 0.0, clip_rect);
            ImGui.TreePop();
        }
    }
    function ShowDemoWindowPopups() {
        ...
        if (ImGui.TreeNode("Popups")) {
            ...
            if (ImGui.BeginPopup("my_select_popup")) {
                ImGui.Text("Aquarium");
                ImGui.Separator();
                for (let i = 0; i < ImGui.IM_ARRAYSIZE(names); i++)
        ...
        if (ImGui.TreeNode("Context menus")) {
            ...
            /* static */ const name = STATIC("name", new ImGui.ImStringBuffer(32, "Label1"));
        ...
        if (ImGui.TreeNode("Modals")) {
            ...
            if (ImGui.BeginPopupModal("Delete?", null, ImGui.ImGuiWindowFlags.AlwaysAutoResize)) {
                ...
                ImGui.PushStyleVar(ImGui.ImGuiStyleVar.FramePadding, new ImGui.ImVec2(0, 0));
                ImGui.Checkbox("Don't ask me next time", (value = dont_ask_me_next_time.value) => dont_ask_me_next_time.value = value);
                ImGui.PopStyleVar();
                if (ImGui.Button("OK", new ImGui.ImVec2(120, 0))) {
                    ImGui.CloseCurrentPopup();
                }
                ImGui.SetItemDefaultFocus();
                ImGui.SameLine();
                if (ImGui.Button("Cancel", new ImGui.ImVec2(120, 0))) {
                    ImGui.CloseCurrentPopup();
                }
                ImGui.EndPopup();
            }
            ...
            if (ImGui.BeginPopupModal("Stacked 1", null, ImGui.ImGuiWindowFlags.MenuBar)) {
                if (ImGui.BeginMenuBar()) {
    ...
    function ShowDemoWindowColumns() {
        ...
        if (ImGui.TreeNode("Basic")) {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                if (ImGui.Selectable(label, selected.value === i, ImGui.ImGuiSelectableFlags.SpanAllColumns))
                    selected.value = i;
        ...
        if (ImGui.TreeNode("Borders")) {
            ...
            for (let i = 0; i < 4 * 3; i++) {
                ...
                ImGui.Button("Button", new ImGui.ImVec2(-1.0, 0.0));
                ImGui.NextColumn();
            }
            ImGui.Columns(1);
            if (h_borders.value)
        ...
        */
        if (ImGui.TreeNode("Horizontal Scrolling")) {
            ImGui.SetNextWindowContentSize(new ImGui.ImVec2(1500.0, 0.0));
            ImGui.BeginChild("##ScrollingRegion", new ImGui.ImVec2(0, ImGui.GetFontSize() * 20), false, ImGui.ImGuiWindowFlags.HorizontalScrollbar);
            ImGui.Columns(10);
            const ITEMS_COUNT = 2000;
            const clipper = new ImGui.ImGuiListClipper(); // Also demonstrate using the clipper for large list
    ...
    function PushStyleCompact() {
        const style = ImGui.GetStyle();
        ImGui.PushStyleVar(ImGui.ImGuiStyleVar.FramePadding, new ImGui.ImVec2(style.FramePadding.x, Math.floor(style.FramePadding.y * 0.60)));
        ImGui.PushStyleVar(ImGui.ImGuiStyleVar.ItemSpacing, new ImGui.ImVec2(style.ItemSpacing.x, Math.floor(style.ItemSpacing.y * 0.60)));
    }
    ...
    function EditTableSizingFlags(p_flags) {
        let policies = [
            { value: ImGui.ImGuiTableFlags.None, name: "Default", tooltip: "Use default sizing policy:\n- ImGuiTableFlags_SizingFixedFit if ScrollX is on or if host window has ImGuiWindowFlags_AlwaysAutoResize.\n- ImGuiTableFlags_SizingStretchSame otherwise." },
            { value: ImGui.ImGuiTableFlags.SizingFixedFit, name: "ImGuiTableFlags_SizingFixedFit", tooltip: "Columns default to _WidthFixed (if resizable) or _WidthAuto (if not resizable), matching contents width." },
            { value: ImGui.ImGuiTableFlags.SizingFixedSame, name: "ImGuiTableFlags_SizingFixedSame", tooltip: "Columns are all the same width, matching the maximum contents width.\nImplicitly disable ImGuiTableFlags_Resizable and enable ImGuiTableFlags_NoKeepColumnsVisible." },
            { value: ImGui.ImGuiTableFlags.SizingStretchProp, name: "ImGuiTableFlags_SizingStretchProp", tooltip: "Columns default to _WidthStretch with weights proportional to their widths." },
            { value: ImGui.ImGuiTableFlags.SizingStretchSame, name: "ImGuiTableFlags_SizingStretchSame", tooltip: "Columns default to _WidthStretch with same weights." }
        ];
        let idx = 0;
        for (idx = 0; idx < ImGui.IM_ARRAYSIZE(policies); idx++)
            if (policies[idx].value == (p_flags.value & ImGui.ImGuiTableFlags.SizingMask_))
                break;
        //const char* preview_text = (idx < IM_ARRAYSIZE(policies)) ? policies[idx].Name + (idx > 0 ? strlen("ImGuiTableFlags") : 0) : "";
        let preview_text = (idx < ImGui.IM_ARRAYSIZE(policies)) ? policies[idx].name.substr(idx > 0 ? "ImGuiTableFlags".length : 0) : "";
        if (ImGui.BeginCombo("Sizing Policy", preview_text)) {
            for (let n = 0; n < ImGui.IM_ARRAYSIZE(policies); n++)
                if (ImGui.Selectable(policies[n].name, idx == n))
                    p_flags.value = (p_flags.value & ~ImGui.ImGuiTableFlags.SizingMask_) | policies[n].value;
            ImGui.EndCombo();
        }
        ImGui.SameLine();
        ImGui.TextDisabled("(?)");
        if (ImGui.IsItemHovered()) {
            ...
            for (let m = 0; m < ImGui.IM_ARRAYSIZE(policies); m++) {
                ImGui.Separator();
    ...
    function EditTableColumnsFlags(p_flags) {
        ImGui.CheckboxFlags("_DefaultHide", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.DefaultHide);
        ImGui.CheckboxFlags("_DefaultSort", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.DefaultSort);
        if (ImGui.CheckboxFlags("_WidthStretch", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.WidthStretch))
            p_flags.value &= ~(ImGui.ImGuiTableColumnFlags.WidthMask_ ^ ImGui.ImGuiTableColumnFlags.WidthStretch);
        if (ImGui.CheckboxFlags("_WidthFixed", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.WidthFixed))
            p_flags.value &= ~(ImGui.ImGuiTableColumnFlags.WidthMask_ ^ ImGui.ImGuiTableColumnFlags.WidthFixed);
        ImGui.CheckboxFlags("_NoResize", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.NoResize);
        ImGui.CheckboxFlags("_NoReorder", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.NoReorder);
        ImGui.CheckboxFlags("_NoHide", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.NoHide);
        ImGui.CheckboxFlags("_NoClip", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.NoClip);
        ImGui.CheckboxFlags("_NoSort", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.NoSort);
        ImGui.CheckboxFlags("_NoSortAscending", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.NoSortAscending);
        ImGui.CheckboxFlags("_NoSortDescending", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.NoSortDescending);
        ImGui.CheckboxFlags("_NoHeaderWidth", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.NoHeaderWidth);
        ImGui.CheckboxFlags("_PreferSortAscending", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.PreferSortAscending);
        ImGui.CheckboxFlags("_PreferSortDescending", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.PreferSortDescending);
        ImGui.CheckboxFlags("_IndentEnable", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.IndentEnable);
        ImGui.SameLine();
        HelpMarker("Default for column 0");
        ImGui.CheckboxFlags("_IndentDisable", (value = p_flags.value) => p_flags.value = value, ImGui.ImGuiTableColumnFlags.IndentDisable);
        ImGui.SameLine();
        HelpMarker("Default for column >0");
    }
    function ShowTableColumnsStatusFlags(flags) {
        const flags_s = [flags];
        ImGui.CheckboxFlags("_IsEnabled", flags_s, ImGui.ImGuiTableColumnFlags.IsEnabled);
        ImGui.CheckboxFlags("_IsVisible", flags_s, ImGui.ImGuiTableColumnFlags.IsVisible);
        ImGui.CheckboxFlags("_IsSorted", flags_s, ImGui.ImGuiTableColumnFlags.IsSorted);
        ImGui.CheckboxFlags("_IsHovered", flags_s, ImGui.ImGuiTableColumnFlags.IsHovered);
    }
    var MyItemColumnID;
...
This is also more similar to the old NextColumn() function of the Columns API, and provided to facilitate the Columns->Tables API transition.`);
        ...
        if (ImGui.TreeNode("Borders, background")) {
            ...
            /* static */ const flags = STATIC("flags#tables2", ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.RowBg);
            ...
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags.RowBg", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.RowBg);
            ImGui.CheckboxFlags("ImGuiTableFlags.Borders", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.Borders);
            ImGui.SameLine();
            HelpMarker("ImGuiTableFlags.Borders\n = ImGuiTableFlags.BordersInnerV\n | ImGuiTableFlags.BordersOuterV\n | ImGuiTableFlags.BordersInnerV\n | ImGuiTableFlags.BordersOuterH");
            ImGui.Indent();
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersH", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.BordersH);
            ImGui.Indent();
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersOuterH", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.BordersOuterH);
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersInnerH", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.BordersInnerH);
            ImGui.Unindent();
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersV", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.BordersV);
            ImGui.Indent();
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersOuterV", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.BordersOuterV);
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersInnerV", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.BordersInnerV);
            ImGui.Unindent();
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersOuter", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.BordersOuter);
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersInner", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.BordersInner);
            ImGui.Unindent();
            ImGui.AlignTextToFramePadding();
            ...
            ImGui.CheckboxFlags("ImGuiTableFlags.NoBordersInBody", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoBordersInBody);
            ImGui.SameLine();
                ...
                for (let row = 0; row < 5; row++) {
                    ...
                    for (let column = 0; column < 3; column++) {
                        ...
                        if (contents_type.value == ContentsType.CT_Text)
                            ...
                            ImGui.Button(`Hello ${column},${row}`, new ImGui.ImVec2(-1.0, 0.0));
                    }
                }
                ImGui.EndTable();
            }
        ...
        if (ImGui.TreeNode("Resizable, stretch")) {
            ...
            /* static */ const flags = STATIC("flags#tables3", ImGui.ImGuiTableFlags.SizingStretchSame | ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.BordersOuter | ImGui.ImGuiTableFlags.BordersV | ImGui.ImGuiTableFlags.ContextMenuInBody);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_Resizable", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.Resizable);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersV", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.BordersV);
            ImGui.SameLine();
...
Double-click a column border to auto-fit the column to its contents.`);
            PushStyleCompact();
            /* static */ const flags = STATIC("flags#tables4", ImGui.ImGuiTableFlags.SizingFixedFit | ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.BordersOuter | ImGui.ImGuiTableFlags.BordersV | ImGui.ImGuiTableFlags.ContextMenuInBody);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoHostExtendX", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoHostExtendX);
            PopStyleCompact();
            if (ImGui.BeginTable("table1", 3, flags.value)) {
                for (let row = 0; row < 5; row++) {
                    ImGui.TableNextRow();
...
When combining Fixed and Stretch columns, generally you only want one, maybe two trailing columns to use _WidthStretch.`);
            const flags = ImGui.ImGuiTableFlags.SizingFixedFit | ImGui.ImGuiTableFlags.RowBg | ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable;
            if (ImGui.BeginTable("table1", 3, flags)) {
                ImGui.TableSetupColumn("AAA", ImGui.ImGuiTableColumnFlags.WidthFixed);
                ImGui.TableSetupColumn("BBB", ImGui.ImGuiTableColumnFlags.WidthFixed);
                ImGui.TableSetupColumn("CCC", ImGui.ImGuiTableColumnFlags.WidthStretch);
                ImGui.TableHeadersRow();
                for (let row = 0; row < 5; row++) {
                    ImGui.TableNextRow();
                    for (let column = 0; column < 3; column++) {
            ...
            if (ImGui.BeginTable("table2", 6, flags)) {
                ImGui.TableSetupColumn("AAA", ImGui.ImGuiTableColumnFlags.WidthFixed);
                ImGui.TableSetupColumn("BBB", ImGui.ImGuiTableColumnFlags.WidthFixed);
                ImGui.TableSetupColumn("CCC", ImGui.ImGuiTableColumnFlags.WidthFixed | ImGui.ImGuiTableColumnFlags.DefaultHide);
                ImGui.TableSetupColumn("DDD", ImGui.ImGuiTableColumnFlags.WidthStretch);
                ImGui.TableSetupColumn("EEE", ImGui.ImGuiTableColumnFlags.WidthStretch);
                ImGui.TableSetupColumn("FFF", ImGui.ImGuiTableColumnFlags.WidthStretch | ImGui.ImGuiTableColumnFlags.DefaultHide);
                ImGui.TableHeadersRow();
                for (let row = 0; row < 5; row++) {
                    ImGui.TableNextRow();
...
Right-click on a header to open a context menu.`);
            /* static */ const flags = STATIC("flags#tables-reorderable", ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable | ImGui.ImGuiTableFlags.BordersOuter | ImGui.ImGuiTableFlags.BordersV);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_Resizable", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.Resizable);
            ImGui.CheckboxFlags("ImGuiTableFlags_Reorderable", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.Reorderable);
            ImGui.CheckboxFlags("ImGuiTableFlags_Hideable", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.Hideable);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoBordersInBody", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoBordersInBody);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoBordersInBodyUntilResize", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoBordersInBodyUntilResize);
            ImGui.SameLine();
            ...
            if (ImGui.BeginTable("table2", 3, flags.value | ImGui.ImGuiTableFlags.SizingFixedFit, new ImGui.ImVec2(0.0, 0.0))) {
                ImGui.TableSetupColumn("One");
                ImGui.TableSetupColumn("Two");
                ImGui.TableSetupColumn("Three");
                ImGui.TableHeadersRow();
...
In this demo we don't show horizontal borders to emphasis how they don't affect default horizontal padding.`);
            /* static */ const flags1 = STATIC("flags1#tables-padding", ImGui.ImGuiTableFlags.BordersV);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_PadOuterX", (value = flags1.value) => flags1.value = value, ImGui.ImGuiTableFlags.PadOuterX);
            ImGui.SameLine();
            HelpMarker("Enable outer-most padding (default if ImGuiTableFlags_BordersOuterV is set)");
            ImGui.CheckboxFlags("ImGuiTableFlags_NoPadOuterX", (value = flags1.value) => flags1.value = value, ImGui.ImGuiTableFlags.NoPadOuterX);
            ImGui.SameLine();
            HelpMarker("Disable outer-most padding (default if ImGuiTableFlags_BordersOuterV is not set)");
            ImGui.CheckboxFlags("ImGuiTableFlags_NoPadInnerX", (value = flags1.value) => flags1.value = value, ImGui.ImGuiTableFlags.NoPadInnerX);
            ImGui.SameLine();
            ...
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersOuterV", (value = flags1.value) => flags1.value = value, ImGui.ImGuiTableFlags.BordersOuterV);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersInnerV", (value = flags1.value) => flags1.value = value, ImGui.ImGuiTableFlags.BordersInnerV);
            ...
            /* static */ const flags2 = STATIC("flags2#tables-padding", ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.RowBg);
            /* static */ const cell_padding = STATIC("cell_padding#tables-padding", new ImGui.ImVec2(0.0, 0.0));
            /* static */ const show_widget_frame_bg = STATIC("const show_widget_frame_bg#tables-padding", true);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_Borders", (value = flags2.value) => flags2.value = value, ImGui.ImGuiTableFlags.Borders);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersH", (value = flags2.value) => flags2.value = value, ImGui.ImGuiTableFlags.BordersH);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersV", (value = flags2.value) => flags2.value = value, ImGui.ImGuiTableFlags.BordersV);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersInner", (value = flags2.value) => flags2.value = value, ImGui.ImGuiTableFlags.BordersInner);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersOuter", (value = flags2.value) => flags2.value = value, ImGui.ImGuiTableFlags.BordersOuter);
            ImGui.CheckboxFlags("ImGuiTableFlags_RowBg", (value = flags2.value) => flags2.value = value, ImGui.ImGuiTableFlags.RowBg);
            ImGui.CheckboxFlags("ImGuiTableFlags_Resizable", (value = flags2.value) => flags2.value = value, ImGui.ImGuiTableFlags.Resizable);
            ...
            PopStyleCompact();
            ImGui.PushStyleVar(ImGui.ImGuiStyleVar.CellPadding, cell_padding.value);
            if (ImGui.BeginTable("table_padding_2", 3, flags2.value)) {
                //static bool init = true;
                if (!show_widget_frame_bg.value)
                    ImGui.PushStyleColor(ImGui.ImGuiCol.FrameBg, 0);
                for (let cell = 0; cell < 3 * 5; cell++) {
                    ImGui.TableNextColumn();
                    ImGui.SetNextItemWidth(-1.0);
                    ImGui.PushID(cell);
                    /* static */ const str0 = STATIC(`str0#${cell}`, new ImGui.ImStringBuffer(16, "edit me"));
                    ImGui.InputText("##cell", str0.value, ImGui.IM_ARRAYSIZE(str0.value));
                    ImGui.PopID();
                }
                if (!show_widget_frame_bg.value)
                    ImGui.PopStyleColor();
        ...
        if (ImGui.TreeNode("Sizing policies")) {
            /* static */ const flags1 = STATIC("flags1#tables-sizing-policies", ImGui.ImGuiTableFlags.BordersV | ImGui.ImGuiTableFlags.BordersOuterH | ImGui.ImGuiTableFlags.RowBg | ImGui.ImGuiTableFlags.ContextMenuInBody);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_Resizable", (value = flags1.value) => flags1.value = value, ImGui.ImGuiTableFlags.Resizable);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoHostExtendX", (value = flags1.value) => flags1.value = value, ImGui.ImGuiTableFlags.NoHostExtendX);
            PopStyleCompact();
            let sizing_policy_flags = [ImGui.ImGuiTableFlags.SizingFixedFit, ImGui.ImGuiTableFlags.SizingFixedSame, ImGui.ImGuiTableFlags.SizingStretchProp, ImGui.ImGuiTableFlags.SizingStretchSame];
            ...
            /* static */ const flags = STATIC("flags#tables-sizing-policies", ImGui.ImGuiTableFlags.ScrollY | ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.RowBg | ImGui.ImGuiTableFlags.Resizable);
            ...
            ImGui.CheckboxFlags("ImGuiTableFlags_Resizable", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.Resizable);
            ImGui.CheckboxFlags("ImGuiTableFlags_PreciseWidths", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.PreciseWidths);
            ImGui.SameLine();
            ...
            ImGui.CheckboxFlags("ImGuiTableFlags_ScrollX", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.ScrollX);
            ImGui.CheckboxFlags("ImGuiTableFlags_ScrollY", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.ScrollY);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoClip", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoClip);
            ImGui.PopItemWidth();
            ImGui.PopID();
            PopStyleCompact();
            if (ImGui.BeginTable("table2", column_count.value, flags.value, new ImGui.ImVec2(0.0, TEXT_BASE_HEIGHT * 7))) {
                for (let cell = 0; cell < 10 * column_count.value; cell++) {
                    ...
                    /* static */ const text_buf = STATIC(`text_buf#sizing_${cell}`, new ImGui.ImStringBuffer(32, ""));
                    switch (contents_type.value) {
                        ...
                        case ContentsType.CT_FillButton:
                            ImGui.Button(label, new ImGui.ImVec2(-1.0, 0.0));
                            break;
                        case ContentsType.CT_InputText:
                            ImGui.SetNextItemWidth(-1.0);
                            ImGui.InputText("##", text_buf.value, ImGui.IM_ARRAYSIZE(text_buf.value));
                            break;
                    }
                    ImGui.PopID();
                }
        ...
        if (ImGui.TreeNode("Vertical scrolling, with clipping")) {
            ...
            /* static */ const flags = STATIC("flags#tables-vertical-scrolling", ImGui.ImGuiTableFlags.ScrollY | ImGui.ImGuiTableFlags.RowBg | ImGui.ImGuiTableFlags.BordersOuter | ImGui.ImGuiTableFlags.BordersV | ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_ScrollY", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.ScrollY);
            PopStyleCompact();
            // When using ScrollX or ScrollY we need to specify a size for our table container!
            // Otherwise by default the table will fit all available space, like a BeginChild() call.
            let outer_size = new ImGui.ImVec2(0.0, TEXT_BASE_HEIGHT * 8);
            if (ImGui.BeginTable("table_scrolly", 3, flags.value, outer_size)) {
                ImGui.TableSetupScrollFreeze(0, 1); // Make top row always visible
                ImGui.TableSetupColumn("One", ImGui.ImGuiTableColumnFlags.None);
                ImGui.TableSetupColumn("Two", ImGui.ImGuiTableColumnFlags.None);
                ImGui.TableSetupColumn("Three", ImGui.ImGuiTableColumnFlags.None);
                ImGui.TableHeadersRow();
                // Demonstrate using clipper for large vertical lists
                let clipper = new ImGui.ImGuiListClipper();
                clipper.Begin(1000);
                while (clipper.Step()) {
...
"because the container window won't automatically extend vertically to fix contents (this may be improved in future versions)."`);
            /* static */ const flags = STATIC("flags#tables-horizontal-scrolling", ImGui.ImGuiTableFlags.ScrollX | ImGui.ImGuiTableFlags.ScrollY | ImGui.ImGuiTableFlags.RowBg | ImGui.ImGuiTableFlags.BordersOuter | ImGui.ImGuiTableFlags.BordersV | ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable);
            ...
            if (ImGui.BeginTable("table_scrollx", 7, flags.value, outer_size)) {
                ...
                ImGui.TableSetupColumn("Line #", ImGui.ImGuiTableColumnFlags.NoHide); // Make the first column not hideable to match our use of TableSetupScrollFreeze()
                ImGui.TableSetupColumn("One");
                ImGui.TableSetupColumn("Two");
                ImGui.TableSetupColumn("Three");
...
Without an explicit value, inner_width is == outer_size.x and therefore using Stretch columns + ScrollX together doesn't make sense.`);
            /* static */ const flags2 = STATIC("flags2#tables-horizontal-scrolling", ImGui.ImGuiTableFlags.SizingStretchSame | ImGui.ImGuiTableFlags.ScrollX | ImGui.ImGuiTableFlags.ScrollY | ImGui.ImGuiTableFlags.BordersOuter | ImGui.ImGuiTableFlags.RowBg | ImGui.ImGuiTableFlags.ContextMenuInBody);
            /* static */ const inner_width = STATIC("inner_width#tables-horizontal-scrolling", 1000.0);
            PushStyleCompact();
            ImGui.PushID("flags3");
            ImGui.PushItemWidth(TEXT_BASE_WIDTH * 30);
            ImGui.CheckboxFlags("ImGuiTableFlags_ScrollX", (value = flags2.value) => flags2.value = value, ImGui.ImGuiTableFlags.ScrollX);
        ...
        if (ImGui.TreeNode("Columns flags")) {
            ...
            let column_flags = [
                ImGui.ImGuiTableColumnFlags.DefaultSort, ImGui.ImGuiTableColumnFlags.None, ImGui.ImGuiTableColumnFlags.DefaultHide
            ...
            if (ImGui.BeginTable("table_columns_flags_checkboxes", column_count, ImGui.ImGuiTableFlags.None)) {
                PushStyleCompact();
            ...
            let flags = ImGui.ImGuiTableFlags.SizingFixedFit | ImGui.ImGuiTableFlags.ScrollX | ImGui.ImGuiTableFlags.ScrollY
                | ImGui.ImGuiTableFlags.RowBg | ImGui.ImGuiTableFlags.BordersOuter | ImGui.ImGuiTableFlags.BordersV
                | ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable | ImGui.ImGuiTableFlags.Sortable;
            let outer_size = new ImGui.ImVec2(0.0, TEXT_BASE_HEIGHT * 9);
        ...
        if (ImGui.TreeNode("Columns widths")) {
            ...
            /* static */ const flags1 = STATIC("flags1#tables-columns-widths", ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.NoBordersInBodyUntilResize);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_Resizable", (value = flags1.value) => flags1.value = value, ImGui.ImGuiTableFlags.Resizable);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoBordersInBodyUntilResize", (value = flags1.value) => flags1.value = value, ImGui.ImGuiTableFlags.NoBordersInBodyUntilResize);
            PopStyleCompact();
            if (ImGui.BeginTable("table1", 3, flags1.value)) {
                // We could also set ImGuiTableFlags_SizingFixedFit on the table and all columns will default to ImGuiTableColumnFlags_WidthFixed.
                ImGui.TableSetupColumn("one", ImGui.ImGuiTableColumnFlags.WidthFixed, 100.0); // Default to 100.0
                ImGui.TableSetupColumn("two", ImGui.ImGuiTableColumnFlags.WidthFixed, 200.0); // Default to 200.0
                ImGui.TableSetupColumn("three", ImGui.ImGuiTableColumnFlags.WidthFixed); // Default to auto
                ImGui.TableHeadersRow();
                for (let row = 0; row < 4; row++) {
                    ImGui.TableNextRow();
                    for (let column = 0; column < 3; column++) {
            ...
            /* static */ const flags2 = STATIC("flags2#tables-columns-widths", ImGui.ImGuiTableFlags.None);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_NoKeepColumnsVisible", (value = flags2.value) => flags2.value = value, ImGui.ImGuiTableFlags.NoKeepColumnsVisible);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersInnerV", (value = flags2.value) => flags2.value = value, ImGui.ImGuiTableFlags.BordersInnerV);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersOuterV", (value = flags2.value) => flags2.value = value, ImGui.ImGuiTableFlags.BordersOuterV);
            PopStyleCompact();
            if (ImGui.BeginTable("table2", 4, flags2.value)) {
                // We could also set ImGuiTableFlags_SizingFixedFit on the table and all columns will default to ImGuiTableColumnFlags_WidthFixed.
                ImGui.TableSetupColumn("", ImGui.ImGuiTableColumnFlags.WidthFixed, 100.0);
                ImGui.TableSetupColumn("", ImGui.ImGuiTableColumnFlags.WidthFixed, TEXT_BASE_WIDTH * 15.0);
                ImGui.TableSetupColumn("", ImGui.ImGuiTableColumnFlags.WidthFixed, TEXT_BASE_WIDTH * 30.0);
                ImGui.TableSetupColumn("", ImGui.ImGuiTableColumnFlags.WidthFixed, TEXT_BASE_WIDTH * 15.0);
                for (let row = 0; row < 5; row++) {
                    ImGui.TableNextRow();
        ...
        if (ImGui.TreeNode("Nested tables")) {
            ...
            if (ImGui.BeginTable("table.nested1", 2, ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable)) {
                ...
                ImGui.Text("A0 Cell 0");
                {
                    ...
                    if (ImGui.BeginTable("table.nested2", 2, ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable)) {
                        ImGui.TableSetupColumn("B0");
                        ImGui.TableSetupColumn("B1");
                        ImGui.TableHeadersRow();
                        ImGui.TableNextRow(ImGui.ImGuiTableRowFlags.None, rows_height);
                        ImGui.TableNextColumn();
        ...
        if (ImGui.TreeNode("Outer size")) {
            ...
            PushStyleCompact();
            /* static */ const flags = STATIC("flags#tables-row-height", ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.ContextMenuInBody | ImGui.ImGuiTableFlags.RowBg | ImGui.ImGuiTableFlags.SizingFixedFit | ImGui.ImGuiTableFlags.NoHostExtendX);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoHostExtendX", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoHostExtendX);
            ImGui.SameLine();
            HelpMarker("Make outer width auto-fit to columns, overriding outer_size.x value.\n\nOnly available when ScrollX/ScrollY are disabled and Stretch columns are not used.");
            ImGui.CheckboxFlags("ImGuiTableFlags_NoHostExtendY", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoHostExtendY);
            ImGui.SameLine();
            ...
            PopStyleCompact();
            let outer_size = new ImGui.ImVec2(0.0, TEXT_BASE_HEIGHT * 5.5);
            ...
            if (ImGui.BeginTable("table2", 3, ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.RowBg, new ImGui.ImVec2(TEXT_BASE_WIDTH * 30, 0.0))) {
                for (let row = 0; row < 5; row++) {
                    ImGui.TableNextRow();
                    for (let column = 0; column < 3; column++) {
                        ImGui.TableNextColumn();
            ...
            ImGui.SameLine();
            if (ImGui.BeginTable("table3", 3, ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.RowBg, new ImGui.ImVec2(TEXT_BASE_WIDTH * 30, 0.0))) {
                for (let row = 0; row < 3; row++) {
        ...
        if (ImGui.TreeNode("Background color")) {
            /* static */ const flags = STATIC("flags#tables-bg", ImGui.ImGuiTableFlags.RowBg);
            ...
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_Borders", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.Borders);
            ImGui.CheckboxFlags("ImGuiTableFlags_RowBg", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.RowBg);
            ImGui.SameLine();
            ...
            if (ImGui.BeginTable("table1", 5, flags.value)) {
                for (let row = 0; row < 6; row++) {
                    ...
                    if (row_bg_type.value != 0) {
                        let row_bg_color = ImGui.GetColorU32(row_bg_type.value == 1 ? new ImGui.ImVec4(0.7, 0.3, 0.3, 0.65) : new ImGui.ImVec4(0.2 + row * 0.1, 0.2, 0.2, 0.65)); // Flat or Gradient?
                        ImGui.TableSetBgColor(ImGui.ImGuiTableBgTarget.RowBg0 + row_bg_target.value, row_bg_color);
                    }
                    // Fill cells
                    for (let column = 0; column < 5; column++) {
                        ...
                        if (row >= 1 && row <= 2 && column >= 1 && column <= 2 && cell_bg_type.value == 1) {
                            let cell_bg_color = ImGui.GetColorU32(new ImGui.ImVec4(0.3, 0.3, 0.7, 0.65));
                            ImGui.TableSetBgColor(ImGui.ImGuiTableBgTarget.CellBg, cell_bg_color);
                        }
                    }
                }
                ImGui.EndTable();
        ...
        if (ImGui.TreeNode("Tree view")) {
            /* static */ const flags = STATIC("flags#tables-tree-view", ImGui.ImGuiTableFlags.BordersV | ImGui.ImGuiTableFlags.BordersOuterH | ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.RowBg | ImGui.ImGuiTableFlags.NoBordersInBody);
            if (ImGui.BeginTable("3ways", 3, flags.value)) {
                // The first column will use the default _WidthStretch when ScrollX is Off and _WidthFixed when ScrollX is On
                ImGui.TableSetupColumn("Name", ImGui.ImGuiTableColumnFlags.NoHide);
                ImGui.TableSetupColumn("Size", ImGui.ImGuiTableColumnFlags.WidthFixed, TEXT_BASE_WIDTH * 12.0);
                ImGui.TableSetupColumn("Type", ImGui.ImGuiTableColumnFlags.WidthFixed, TEXT_BASE_WIDTH * 18.0);
                ImGui.TableHeadersRow();
                ...
                class MyTreeNode {
                    ...
                    static DisplayNode(node, all_nodes) {
                        ...
                        if (is_folder) {
                            let open = ImGui.TreeNodeEx(node.Name, ImGui.ImGuiTreeNodeFlags.SpanFullWidth);
                            ImGui.TableNextColumn();
                            ImGui.TextDisabled("--");
                            ImGui.TableNextColumn();
                            ...
                            ImGui.TreeNodeEx(node.Name, ImGui.ImGuiTreeNodeFlags.Leaf | ImGui.ImGuiTreeNodeFlags.Bullet | ImGui.ImGuiTreeNodeFlags.NoTreePushOnOpen | ImGui.ImGuiTreeNodeFlags.SpanFullWidth);
                            ImGui.TableNextColumn();
        ...
        if (ImGui.TreeNode("Item width")) {
            ...
            if (ImGui.BeginTable("table_item_width", 3, ImGui.ImGuiTableFlags.Borders)) {
        ...
        if (ImGui.TreeNode("Custom headers")) {
            ...
            if (ImGui.BeginTable("table_custom_headers", COLUMNS_COUNT, ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable)) {
        ...
        if (ImGui.TreeNode("Context menus")) {
            ...
            /* static */ const flags1 = STATIC("flags1#tables-context-menus", ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable | ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.ContextMenuInBody);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_ContextMenuInBody", (value = flags1.value) => flags1.value = value, ImGui.ImGuiTableFlags.ContextMenuInBody);
            PopStyleCompact();
            ...
            /* static */ const flags2 = STATIC("flags2#tables-context-menus", ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.SizingFixedFit | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable | ImGui.ImGuiTableFlags.Borders);
            if (ImGui.BeginTable("table_context_menu_2", COLUMNS_COUNT, flags2.value)) {
                ...
                for (let column = 0; column < COLUMNS_COUNT + 1; column++) {
                    ImGui.PushID(column);
                    if (ImGui.TableGetColumnFlags(column) & ImGui.ImGuiTableColumnFlags.IsHovered)
        ...
        if (ImGui.TreeNode("Synced instances")) {
            ...
            for (let n = 0; n < 3; n++) {
                ...
                let open = ImGui.CollapsingHeader(buf, ImGui.ImGuiTreeNodeFlags.DefaultOpen);
                if (open && ImGui.BeginTable("Table", 3, ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable | ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.SizingFixedFit | ImGui.ImGuiTableFlags.NoSavedSettings)) {
                    ImGui.TableSetupColumn("One");
                    ImGui.TableSetupColumn("Two");
                    ImGui.TableSetupColumn("Three");
                    ImGui.TableHeadersRow();
        ...
        if (ImGui.TreeNode("Sorting")) {
            // Options
            /* static */ const flags = STATIC("flags#tables-sorting", ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable | ImGui.ImGuiTableFlags.Sortable | ImGui.ImGuiTableFlags.SortMulti | ImGui.ImGuiTableFlags.RowBg | ImGui.ImGuiTableFlags.BordersOuter | ImGui.ImGuiTableFlags.BordersV | ImGui.ImGuiTableFlags.NoBordersInBody | ImGui.ImGuiTableFlags.ScrollY);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_SortMulti", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.SortMulti);
            ImGui.SameLine();
            ...
            ImGui.CheckboxFlags("ImGuiTableFlags_SortTristate", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.SortTristate);
            ImGui.SameLine();
            ...
            PopStyleCompact();
            if (ImGui.BeginTable("table_sorting", 4, flags.value, new ImGui.ImVec2(0.0, TEXT_BASE_HEIGHT * 15), 0.0)) {
                // Declare columns
                ...
                ImGui.TableSetupColumn("ID", ImGui.ImGuiTableColumnFlags.DefaultSort | ImGui.ImGuiTableColumnFlags.WidthFixed, 0.0, MyItemColumnID.ID);
                ImGui.TableSetupColumn("Name", ImGui.ImGuiTableColumnFlags.WidthFixed, 0.0, MyItemColumnID.Name);
                ImGui.TableSetupColumn("Action", ImGui.ImGuiTableColumnFlags.NoSort | ImGui.ImGuiTableColumnFlags.WidthFixed, 0.0, MyItemColumnID.Action);
                ImGui.TableSetupColumn("Quantity", ImGui.ImGuiTableColumnFlags.PreferSortDescending | ImGui.ImGuiTableColumnFlags.WidthStretch, 0.0, MyItemColumnID.Quantity);
                        ...
                        if (table_sort_items.value.length > 1)
                            table_sort_items.value.sort((a, b) => {
                                ...
                                for (let n = 0; n < sort_specs.SpecsCount; n++) {
                                    ...
                                    if (delta > 0)
                                        return sort_spec.SortDirection == ImGui.ImGuiSortDirection.Ascending ? 1 : -1;
                                    if (delta < 0)
                                        return sort_spec.SortDirection == ImGui.ImGuiSortDirection.Ascending ? -1 : 1;
                                }
        ...
        if (ImGui.TreeNode("Advanced")) {
            /* static */ const flags = STATIC("flags#tables-sorting-advanced", ImGui.ImGuiTableFlags.Resizable | ImGui.ImGuiTableFlags.Reorderable | ImGui.ImGuiTableFlags.Hideable | ImGui.ImGuiTableFlags.Sortable | ImGui.ImGuiTableFlags.SortMulti | ImGui.ImGuiTableFlags.RowBg | ImGui.ImGuiTableFlags.Borders | ImGui.ImGuiTableFlags.NoBordersInBody | ImGui.ImGuiTableFlags.ScrollX | ImGui.ImGuiTableFlags.ScrollY | ImGui.ImGuiTableFlags.SizingFixedFit);
            let ContentsType;
            (function (ContentsType) {
            ...
            const outer_size_value = STATIC("outer_size_value#tables-sorting-advanced", new ImGui.ImVec2(0.0, TEXT_BASE_HEIGHT * 12));
            ...
            if (ImGui.TreeNode("Options")) {
                ...
                if (ImGui.TreeNodeEx("Features:", ImGui.ImGuiTreeNodeFlags.DefaultOpen)) {
                    ImGui.CheckboxFlags("ImGuiTableFlags.Resizable", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.Resizable);
                    ImGui.CheckboxFlags("ImGuiTableFlags.Reorderable", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.Reorderable);
                    ImGui.CheckboxFlags("ImGuiTableFlags.Hideable", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.Hideable);
                    ImGui.CheckboxFlags("ImGuiTableFlags.Sortable", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.Sortable);
                    ImGui.CheckboxFlags("ImGuiTableFlags.NoSavedSettings", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoSavedSettings);
                    ImGui.CheckboxFlags("ImGuiTableFlags.ContextMenuInBody", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.ContextMenuInBody);
                    ImGui.TreePop();
                }
                if (ImGui.TreeNodeEx("Decorations:", ImGui.ImGuiTreeNodeFlags.DefaultOpen)) {
                    ImGui.CheckboxFlags("ImGuiTableFlags.RowBg", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.RowBg);
                    ImGui.CheckboxFlags("ImGuiTableFlags.BordersV", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.BordersV);
                    ImGui.CheckboxFlags("ImGuiTableFlags.BordersOuterV", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.BordersOuterV);
                    ImGui.CheckboxFlags("ImGuiTableFlags.BordersInnerV", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.BordersInnerV);
                    ...
                    ImGui.SameLine();
                    HelpMarker("Disable vertical borders in columns Body (borders will always appears in Headers");
                    ImGui.CheckboxFlags("ImGuiTableFlags.NoBordersInBodyUntilResize", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoBordersInBodyUntilResize);
                    ImGui.SameLine();
                    HelpMarker("Disable vertical borders in columns Body until hovered for resize (borders will always appears in Headers)");
                    ImGui.TreePop();
                }
                if (ImGui.TreeNodeEx("Sizing:", ImGui.ImGuiTreeNodeFlags.DefaultOpen)) {
                    EditTableSizingFlags(flags);
                    ImGui.SameLine();
                    ...
                    ImGui.CheckboxFlags("ImGuiTableFlags.NoHostExtendX", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoHostExtendX);
                    ImGui.SameLine();
                    HelpMarker("Make outer width auto-fit to columns, overriding outer_size.x value.\n\nOnly available when ScrollX/ScrollY are disabled and Stretch columns are not used.");
                    ImGui.CheckboxFlags("ImGuiTableFlags_NoHostExtendY", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoHostExtendY);
                    ImGui.SameLine();
                    HelpMarker("Make outer height stop exactly at outer_size.y (prevent auto-extending table past the limit).\n\nOnly available when ScrollX/ScrollY are disabled. Data below the limit will be clipped and not visible.");
                    ImGui.CheckboxFlags("ImGuiTableFlags_NoKeepColumnsVisible", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoKeepColumnsVisible);
                    ImGui.SameLine();
                ...
                if (ImGui.TreeNodeEx("Padding:", ImGui.ImGuiTreeNodeFlags.DefaultOpen)) {
                    ImGui.CheckboxFlags("ImGuiTableFlags.PadOuterX", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.PadOuterX);
                    ImGui.CheckboxFlags("ImGuiTableFlags.NoPadOuterX", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoPadOuterX);
                    ImGui.CheckboxFlags("ImGuiTableFlags.NoPadInnerX", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.NoPadInnerX);
                    ImGui.TreePop();
                }
                if (ImGui.TreeNodeEx("Scrolling:", ImGui.ImGuiTreeNodeFlags.DefaultOpen)) {
                    ImGui.CheckboxFlags("ImGuiTableFlags.ScrollX", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.ScrollX);
                    ImGui.SameLine();
                    ImGui.SetNextItemWidth(ImGui.GetFrameHeight());
                    ImGui.DragInt("freeze_cols", (value = freeze_cols.value) => freeze_cols.value = value, 0.2, 0, 9);
                    ImGui.CheckboxFlags("ImGuiTableFlags.ScrollY", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.ScrollY);
                    ImGui.SameLine();
                    ImGui.SetNextItemWidth(ImGui.GetFrameHeight());
                    ImGui.DragInt("freeze_rows", (value = freeze_rows.value) => freeze_rows.value = value, 0.2, 0, 9);
                    ImGui.TreePop();
                }
                if (ImGui.TreeNodeEx("Sorting:", ImGui.ImGuiTreeNodeFlags.DefaultOpen)) {
                    ImGui.CheckboxFlags("ImGuiTableFlags.SortMulti", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.SortMulti);
                    ImGui.SameLine();
                    HelpMarker("When sorting is enabled: hold shift when clicking headers to sort on multiple column. TableGetSortSpecs() may return specs where (SpecsCount > 1).");
                    ImGui.CheckboxFlags("ImGuiTableFlags.SortTristate", (value = flags.value) => flags.value = value, ImGui.ImGuiTableFlags.SortTristate);
                    ImGui.SameLine();
                    HelpMarker("When sorting is enabled: allow no sorting, disable default sorting. TableGetSortSpecs() may return specs where (SpecsCount == 0).");
                    ImGui.TreePop();
                }
                if (ImGui.TreeNodeEx("Other:", ImGui.ImGuiTreeNodeFlags.DefaultOpen)) {
                    ImGui.Checkbox("show_headers", (value = show_headers.value) => show_headers.value = value);
                    ...
                    ImGui.Combo("items_type (first column)", (value = contents_type.value) => contents_type.value = value, contents_type_names, ImGui.IM_ARRAYSIZE(contents_type_names));
                    //filter.Draw("filter");
                    ImGui.TreePop();
                }
                ImGui.PopItemWidth();
            ...
            if (items.value.length != items_count.value) {
                ...
                items.value = Array
                    ...
                    .map((_, n) => {
                    const template_n = n % ImGui.IM_ARRAYSIZE(template_items_names);
            ...
            // for debug display
            let table_scroll_cur = new ImGui.ImVec2(0.0, 0.0);
            let table_scroll_max = new ImGui.ImVec2(0.0, 0.0);
            let table_draw_list = null; // "
            const inner_width_to_use = (flags.value & ImGui.ImGuiTableFlags.ScrollX) ? inner_width_with_scroll.value : 0.0;
            if (ImGui.BeginTable("table_advanced", 6, flags.value, outer_size_enabled.value ? outer_size_value.value : new ImGui.ImVec2(0, 0), inner_width_to_use)) {
                // Declare columns
                // We use the "user_id" parameter of TableSetupColumn() to specify a user id that will be stored in the sort specifications.
                // This is so our sort function can identify a column given our own identifier. We could also identify them based on their index!
                ImGui.TableSetupColumn("ID", ImGui.ImGuiTableColumnFlags.DefaultSort | ImGui.ImGuiTableColumnFlags.WidthFixed | ImGui.ImGuiTableColumnFlags.NoHide, 0.0, MyItemColumnID.ID);
                ImGui.TableSetupColumn("Name", ImGui.ImGuiTableColumnFlags.WidthFixed, 0.0, MyItemColumnID.Name);
                ImGui.TableSetupColumn("Action", ImGui.ImGuiTableColumnFlags.NoSort | ImGui.ImGuiTableColumnFlags.WidthFixed, 0.0, MyItemColumnID.Action);
                ImGui.TableSetupColumn("Quantity", ImGui.ImGuiTableColumnFlags.PreferSortDescending, 0.0, MyItemColumnID.Quantity);
                ImGui.TableSetupColumn("Description", (flags.value & ImGui.ImGuiTableFlags.NoHostExtendX) ? 0 : ImGui.ImGuiTableColumnFlags.WidthStretch, 0.0, MyItemColumnID.Description);
                ImGui.TableSetupColumn("Hidden", ImGui.ImGuiTableColumnFlags.DefaultHide | ImGui.ImGuiTableColumnFlags.NoSort);
                ImGui.TableSetupScrollFreeze(freeze_cols.value, freeze_rows.value);
                // Sort our data if sort specs have been changed!
                ...
                const sorts_specs_using_quantity = (ImGui.TableGetColumnFlags(3) & ImGui.ImGuiTableColumnFlags.IsSorted) != 0;
                // Show headers
                if (show_headers.value)
                    ImGui.TableHeadersRow();
                // Show data
                ...
                while (clipper.Step()) {
                    for (let row_n = clipper.DisplayStart; row_n < clipper.DisplayEnd; row_n++) {
                        ...
                        ImGui.PushID(item.ID);
                        ImGui.TableNextRow(ImGui.ImGuiTableRowFlags.None, row_min_height.value);
                        ImGui.TableNextColumn();
                        ...
                        if (contents_type.value == ContentsType.Text)
                            ...
                            const selectable_flags = (contents_type.value == ContentsType.SelectableSpanRow) ? ImGui.ImGuiSelectableFlags.SpanAllColumns | ImGui.ImGuiSelectableFlags.AllowItemOverlap : ImGui.ImGuiSelectableFlags.None;
                            if (ImGui.Selectable(label, item_is_selected, selectable_flags, new ImGui.ImVec2(0, row_min_height.value))) {
                                if (ImGui.GetIO().KeyCtrl) {
                                    if (item_is_selected)
                ...
                table_scroll_cur = new ImGui.ImVec2(ImGui.GetScrollX(), ImGui.GetScrollY());
                table_scroll_max = new ImGui.ImVec2(ImGui.GetScrollMaxX(), ImGui.GetScrollMaxY());
    ...
    function ShowDemoWindowMisc() {
        if (ImGui.CollapsingHeader("Filtering")) {
            /* static */ const filter = STATIC("filter#1864", new ImGui.ImGuiTextFilter());
            ...
            for (let i = 0; i < ImGui.IM_ARRAYSIZE(lines); i++)
        ...
        if (ImGui.CollapsingHeader("Inputs, Navigation & Focus")) {
            ...
            if (ImGui.TreeNode("Keyboard, Mouse & Navigation State")) {
                ...
                for (let i = 0; i < ImGui.IM_ARRAYSIZE(io.NavInputs); i++)
            ...
            if (ImGui.TreeNode("Tabbing")) {
                ...
                /* static */ const buf = STATIC("buf1#1921", new ImGui.ImStringBuffer(32, "dummy"));
                ImGui.InputText("1", buf.value, ImGui.IM_ARRAYSIZE(buf.value));
                ImGui.InputText("2", buf.value, ImGui.IM_ARRAYSIZE(buf.value));
                ImGui.InputText("3", buf.value, ImGui.IM_ARRAYSIZE(buf.value));
                ImGui.PushAllowKeyboardFocus(false);
                ImGui.InputText("4 (tab skip)", buf.value, ImGui.IM_ARRAYSIZE(buf.value));
                //ImGui.SameLine(); HelpMarker("Use ImGui.PushAllowKeyboardFocus(bool)\nto disable tabbing through certain widgets.");
                ImGui.PopAllowKeyboardFocus();
                ImGui.InputText("5", buf.value, ImGui.IM_ARRAYSIZE(buf.value));
                ImGui.TreePop();
            }
            if (ImGui.TreeNode("Focus from code")) {
                ...
                let has_focus = 0;
                /* static */ const buf = STATIC("buf2#1944", new ImGui.ImStringBuffer(128, "click on a button to set focus"));
                if (focus_1)
                ...
                if (focus_3)
                    ImGui.SetKeyboardFocusHere();
                ImGui.InputText("3 (tab skip)", buf.value, ImGui.IM_ARRAYSIZE(buf.value));
                if (ImGui.IsItemActive())
                    has_focus = 3;
                ImGui.PopAllowKeyboardFocus();
                if (has_focus)
            ...
            if (ImGui.TreeNode("Dragging")) {
                ...
                if (ImGui.IsItemActive())
                    ImGui.GetForegroundDrawList().AddLine(io.MouseClickedPos[0], io.MousePos, ImGui.GetColorU32(ImGui.ImGuiCol.Button), 4.0); // Draw a line between the button and the mouse cursor
            ...
            if (ImGui.TreeNode("Mouse cursors")) {
                ...
                ImGui.IM_ASSERT(ImGui.IM_ARRAYSIZE(mouse_cursors_names) === ImGui.ImGuiMouseCursor.COUNT);
                ...
                for (let i = 0; i < ImGui.ImGuiMouseCursor.COUNT; i++) {
    ...
    function ShowAboutWindow(p_open) {
        if (!ImGui.Begin("About Dear ImGui", p_open, ImGui.ImGuiWindowFlags.AlwaysAutoResize)) {
            ImGui.End();
            return;
        }
    ...
    function ShowStyleEditor(ref = null) {
        ...
        const style = ImGui.GetStyle();
        /* static */ const ref_saved_style = STATIC("ref_saved_style", new ImGui.ImGuiStyle());
        ...
        ImGui.Separator();
        if (ImGui.BeginTabBar("##tabs", ImGui.ImGuiTabBarFlags.None)) {
            ...
            if (ImGui.BeginTabItem("Colors")) {
                ...
                /* static */ const filter = STATIC("filter#2223", new ImGui.ImGuiTextFilter());
                ...
                ImGui.SameLine();
                ImGui.RadioButton("Alpha", (value = alpha_flags.value) => alpha_flags.value = value, ImGui.ImGuiColorEditFlags.AlphaPreview);
                ImGui.SameLine();
                ImGui.RadioButton("Both", (value = alpha_flags.value) => alpha_flags.value = value, ImGui.ImGuiColorEditFlags.AlphaPreviewHalf);
                ImGui.BeginChild("#colors", new ImGui.ImVec2(0, 300), true, ImGui.ImGuiWindowFlags.AlwaysVerticalScrollbar | ImGui.ImGuiWindowFlags.AlwaysHorizontalScrollbar | ImGui.ImGuiWindowFlags.NavFlattened);
                ...
                for (let i = 0; i < ImGui.ImGuiCol.COUNT; i++) {
                    ...
                    if (!filter.value.PassFilter(name))
                        continue;
                    ImGui.PushID(i);
                    ImGui.ColorEdit4("##color", style.Colors[i], ImGui.ImGuiColorEditFlags.AlphaBar | alpha_flags.value);
            ...
            if (ImGui.BeginTabItem("Fonts")) {
                        ...
                        if (ImGui.TreeNode("Glyphs", `Glyphs (${font.Glyphs.Size})`)) {
                            ...
                            for (let base = 0; base < 0x10000; base += 256) {
                                ...
                                if (count > 0 && ImGui.TreeNode(base, `U+${format_number_hex(base, 4).toUpperCase()}..U+${(format_number_hex(base + 255, 4).toUpperCase())} (${count} ${count > 1 ? "glyphs" : "glyph"})`)) {
                                    ...
                                    for (let n = 0; n < 256; n++) {
                                        const cell_p1 = new ImGui.ImVec2(base_pos.x + (n % 16) * (cell_size + cell_spacing), base_pos.y + (0 | (n / 16)) * (cell_size + cell_spacing));
                                        const cell_p2 = new ImGui.ImVec2(cell_p1.x + cell_size, cell_p1.y + cell_size);
                                        ...
                                        if (glyph)
                                            font.RenderChar(draw_list, cell_size, cell_p1, ImGui.GetColorU32(ImGui.ImGuiCol.Text), (base + n)); // We use ImFont.RenderChar as a shortcut because we don't have UTF-8 conversion functions available to generate a string.
                                        if (glyph && ImGui.IsWindowHovered() && ImGui.IsMouseHoveringRect(cell_p1, cell_p2)) {
                                            ...
                                            ImGui.Separator();
                                            ImGui.Image(ImGui.GetIO().Fonts.TexID, new ImGui.ImVec2(8 * (glyph.X1 - glyph.X0), 8 * (glyph.Y1 - glyph.Y0)), new ImGui.ImVec2(glyph.U0, glyph.V0), new ImGui.ImVec2(glyph.U1, glyph.V1), new ImGui.ImColor(255, 255, 255, 255).toImVec4(), new ImGui.ImColor(255, 255, 255, 128).toImVec4());
                                            ImGui.SameLine();
                                            ImGui.BeginGroup();
                                    ...
                                    ImGui.Dummy(new ImGui.ImVec2((cell_size + cell_spacing) * 16, (cell_size + cell_spacing) * 16));
                                    ImGui.TreePop();
                                }
                            }
                            ImGui.TreePop();
                ...
                if (ImGui.TreeNode("Atlas texture", `Atlas texture (${atlas.TexWidth}x${atlas.TexHeight} pixels)`)) {
                    const tint_col = new ImGui.ImVec4(1.0, 1.0, 1.0, 1.0);
                    const border_col = new ImGui.ImVec4(1.0, 1.0, 1.0, 0.5);
                    ImGui.Image(atlas.TexID, new ImGui.ImVec2(atlas.TexWidth, atlas.TexHeight), new ImGui.ImVec2(0, 0), new ImGui.ImVec2(1, 1), tint_col, border_col);
                    ImGui.TreePop();
                }
    ...
    function ShowExampleMenuFile() {
        ...
        if (ImGui.BeginMenu("Options")) {
            ...
            ImGui.BeginChild("child", new ImGui.ImVec2(0, 60), true);
            for (let i = 0; i < 10; i++)
        ...
        if (ImGui.BeginMenu("Colors")) {
            ...
            for (let i = 0; i < ImGui.ImGuiCol.COUNT; i++) {
                ...
                ImGui.GetWindowDrawList().AddRectFilled(p, new ImGui.ImVec2(p.x + sz, p.y + sz), ImGui.GetColorU32(i));
                ImGui.Dummy(new ImGui.ImVec2(sz, sz));
                ImGui.SameLine();
                ImGui.MenuItem(name);
            }
            ImGui.EndMenu();
    ...
    class ExampleAppConsole {
        constructor() {
            // char                  InputBuf[256];
            this.InputBuf = new ImGui.ImStringBuffer(256, "");
            // ImVector<char*>       Items;
            this.Items = new ImGui.ImVector();
            // ImVector<const char*> Commands;
            this.Commands = new ImGui.ImVector();
            // ImVector<char*>       History;
            this.History = new ImGui.ImVector();
            // int                   HistoryPos;    // -1: new line, 0..History.Size-1 browsing history.
            this.HistoryPos = -1;
            // ImGuiTextFilter       Filter;
            this.Filter = new ImGui.ImGuiTextFilter();
            // bool                  AutoScroll;
        ...
        Draw(title, p_open) {
            ImGui.SetNextWindowSize(new ImGui.ImVec2(520, 600), ImGui.ImGuiCond.FirstUseEver);
            ...
            ImGui.BeginChild("ScrollingRegion", new ImGui.ImVec2(0, -footer_height_to_reserve), false, ImGui.ImGuiWindowFlags.HorizontalScrollbar); // Leave room for 1 separator + 1 InputText
            ...
            ImGui.PushStyleVar(ImGui.ImGuiStyleVar.ItemSpacing, new ImGui.ImVec2(4, 1)); // Tighten spacing
            if (copy_to_clipboard)
                ImGui.LogToClipboard();
            for (let i = 0; i < this.Items.Size; i++) {
                ...
                if (/\[error\]/.test(item)) {
                    ImGui.PushStyleColor(ImGui.ImGuiCol.Text, new ImGui.ImVec4(1.0, 0.4, 0.4, 1.0));
                    pop_color = true;
                }
                // else if (strncmp(item, "# ", 2) == 0)   { ImGui.PushStyleColor(ImGuiCol_Text, new ImVec4(1.0f, 0.8f, 0.6f, 1.0f)); pop_color = true; }
                else if (/^# /.test(item)) {
                    ImGui.PushStyleColor(ImGui.ImGuiCol.Text, new ImGui.ImVec4(1.0, 0.8, 0.6, 1.0));
                    pop_color = true;
                }
            ...
            if (ImGui.InputText("Input", this.InputBuf, ImGui.IM_ARRAYSIZE(this.InputBuf), ImGui.ImGuiInputTextFlags.EnterReturnsTrue | ImGui.ImGuiInputTextFlags.CallbackCompletion | ImGui.ImGuiInputTextFlags.CallbackHistory, ExampleAppConsole.TextEditCallbackStub, this)) {
                // char* s = InputBuf;
                // Strtrim(s);
                // if (s[0])
                //     ExecCommand(s);
        ...
        TextEditCallback(data) {
            ...
            switch (data.EventFlag) {
                case ImGui.ImGuiInputTextFlags.CallbackCompletion:
                    {
                ...
                case ImGui.ImGuiInputTextFlags.CallbackHistory:
            }
            return 0;
        }
    }
    ...
    class ExampleAppLog {
        constructor() {
            // ImGuiTextBuffer     Buf;
            this.Buf = new ImGui.ImGuiTextBuffer();
            // ImGuiTextFilter     Filter;
            this.Filter = new ImGui.ImGuiTextFilter();
            // ImVector<int>       LineOffsets;        // Index to lines offset. We maintain this with AddLog() calls, allowing us to have a random access on lines
            this.LineOffsets = new ImGui.ImVector();
            // bool                AutoScroll;
            this.AutoScroll = true;
            // bool                ScrollToBottom;
            this.ScrollToBottom = false;
        ...
        Draw(title, p_open) {
            ImGui.SetNextWindowSize(new ImGui.ImVec2(500, 400), ImGui.ImGuiCond.FirstUseEver);
            if (!ImGui.Begin(title, p_open)) {
                ImGui.End();
                return;
            }
            ...
            ImGui.Separator();
            ImGui.BeginChild("scrolling", new ImGui.ImVec2(0, 0), false, ImGui.ImGuiWindowFlags.HorizontalScrollbar);
            if (clear)
                this.Clear();
            if (copy)
                ImGui.LogToClipboard();
    ...
    function ShowExampleAppLog(p_open) {
        ...
        if (ImGui.SmallButton("[Debug] Add 5 entries")) {
            ...
            for (let n = 0; n < 5; n++) {
                ...
                log.value.AddLog(`[${ImGui.GetFrameCount()}] [${categories[counter.value % ImGui.IM_ARRAYSIZE(categories)]}] Hello, current time is ${ImGui.GetTime()}, here's a word: '${words[counter.value % ImGui.IM_ARRAYSIZE(words)]}'\n`);
                counter.value++;
            }
        }
        ImGui.End();
    ...
    function ShowExampleAppLayout(p_open) {
        ImGui.SetNextWindowSize(new ImGui.ImVec2(500, 440), ImGui.ImGuiCond.FirstUseEver);
        if (ImGui.Begin("Example: Simple Layout", p_open, ImGui.ImGuiWindowFlags.MenuBar)) {
            ...
            ImGui.BeginChild("left pane", new ImGui.ImVec2(150, 0), true);
            for (let i = 0; i < 100; i++) {
            ...
            // right
            ImGui.BeginGroup();
            ImGui.BeginChild("item view", new ImGui.ImVec2(0, -ImGui.GetFrameHeightWithSpacing())); // Leave room for 1 line below us
    ...
    function ShowExampleAppPropertyEditor(p_open) {
        ImGui.SetNextWindowSize(new ImGui.ImVec2(430, 450), ImGui.ImGuiCond.FirstUseEver);
        ...
        ImGui.PushStyleVar(ImGui.ImGuiStyleVar.FramePadding, new ImGui.ImVec2(2, 2));
        ImGui.Columns(2);
        ImGui.Separator();
        class funcs {
                ...
                if (node_open) {
                    ...
                    for (let i = 0; i < 8; i++) {
                        ...
                        if (i < 2) {
                            ...
                            ImGui.TreeNodeEx("Field", ImGui.ImGuiTreeNodeFlags.Leaf | ImGui.ImGuiTreeNodeFlags.NoTreePushOnOpen | ImGui.ImGuiTreeNodeFlags.Bullet, `Field_${i}`);
                            ImGui.NextColumn();
                            ImGui.SetNextItemWidth(-1);
    ...
    function ShowExampleAppLongText(p_open) {
        ...
        /* static */ const log = STATIC("log#3217", new ImGui.ImGuiTextBuffer());
    ...
    function ShowExampleAppConstrainedResize(p_open) {
        ...
        if (type.value === 0)
            ImGui.SetNextWindowSizeConstraints(new ImGui.ImVec2(-1, 0), new ImGui.ImVec2(-1, Number.MAX_VALUE)); // Vertical only
        if (type.value === 1)
            ImGui.SetNextWindowSizeConstraints(new ImGui.ImVec2(0, -1), new ImGui.ImVec2(Number.MAX_VALUE, -1)); // Horizontal only
        if (type.value === 2)
            ImGui.SetNextWindowSizeConstraints(new ImGui.ImVec2(100, 100), new ImGui.ImVec2(Number.MAX_VALUE, Number.MAX_VALUE)); // Width > 100, Height > 100
        if (type.value === 3)
            ImGui.SetNextWindowSizeConstraints(new ImGui.ImVec2(400, -1), new ImGui.ImVec2(500, -1)); // Width 400-500
        if (type.value === 4)
            ImGui.SetNextWindowSizeConstraints(new ImGui.ImVec2(-1, 400), new ImGui.ImVec2(-1, 500)); // Height 400-500
        if (type.value === 5)
            ImGui.SetNextWindowSizeConstraints(new ImGui.ImVec2(0, 0), new ImGui.ImVec2(Number.MAX_VALUE, Number.MAX_VALUE), CustomConstraints.Square); // Always Square
        if (type.value === 6)
            ImGui.SetNextWindowSizeConstraints(new ImGui.ImVec2(0, 0), new ImGui.ImVec2(Number.MAX_VALUE, Number.MAX_VALUE), CustomConstraints.Step, 100); // Fixed Step
        const flags = auto_resize.value ? ImGui.ImGuiWindowFlags.AlwaysAutoResize : 0;
        if (ImGui.Begin("Example: Constrained Resize", p_open, flags)) {
            ...
            if (ImGui.Button("200x200")) {
                ImGui.SetWindowSize(new ImGui.ImVec2(200, 200));
            }
            ImGui.SameLine();
            if (ImGui.Button("500x500")) {
                ImGui.SetWindowSize(new ImGui.ImVec2(500, 500));
            }
            ImGui.SameLine();
            if (ImGui.Button("800x200")) {
                ImGui.SetWindowSize(new ImGui.ImVec2(800, 200));
            }
            ImGui.SetNextItemWidth(200);
            ImGui.Combo("Constraint", (value = type.value) => type.value = value, desc, ImGui.IM_ARRAYSIZE(desc));
            ImGui.SetNextItemWidth(200);
    ...
    function ShowExampleAppSimpleOverlay(p_open) {
        ...
        const io = ImGui.GetIO();
        if (corner.value !== -1) {
            const window_pos = new ImGui.ImVec2((corner.value & 1) ? io.DisplaySize.x - DISTANCE : DISTANCE, (corner.value & 2) ? io.DisplaySize.y - DISTANCE : DISTANCE);
            const window_pos_pivot = new ImGui.ImVec2((corner.value & 1) ? 1.0 : 0.0, (corner.value & 2) ? 1.0 : 0.0);
            ImGui.SetNextWindowPos(window_pos, ImGui.ImGuiCond.Always, window_pos_pivot);
        }
        ImGui.SetNextWindowBgAlpha(0.35); // Transparent background
        if (ImGui.Begin("Example: Simple overlay", p_open, (corner.value !== -1 ? ImGui.ImGuiWindowFlags.NoMove : 0) | ImGui.ImGuiWindowFlags.NoDecoration | ImGui.ImGuiWindowFlags.AlwaysAutoResize | ImGui.ImGuiWindowFlags.NoSavedSettings)) {
    ...
    function ShowExampleAppWindowTitles(p_open) {
        ...
        ImGui.End();
        ImGui.SetNextWindowPos(new ImGui.ImVec2(100, 200), ImGui.ImGuiCond.FirstUseEver);
        ...
        ImGui.SetNextWindowPos(new ImGui.ImVec2(100, 300), ImGui.ImGuiCond.FirstUseEver);
        ImGui.Begin(buf);
    ...
    function ShowExampleAppCustomRendering(p_open) {
        ImGui.SetNextWindowSize(new ImGui.ImVec2(350, 560), ImGui.ImGuiCond.FirstUseEver);
        ...
        if (ImGui.BeginTabBar("##TabBar")) {
            ...
            if (ImGui.BeginTabItem("Primitives")) {
                ...
                /* static */ const col = STATIC("color#2583", new ImGui.ImVec4(1.0, 1.0, 0.4, 1.0));
                ...
                {
                    const p = ImGui.GetCursorScreenPos();
                    const col32 = ImGui.IM_COL32(col.value.x * 255, col.value.y * 255, col.value.z * 255, col.value.w * 255);
                    let x = p.x + 4.0, y = p.y + 4.0;
                    const spacing = 8.0;
                    for (let n = 0; n < 2; n++) {
                        const curr_thickness = (n === 0) ? 1.0 : thickness.value;
                        draw_list.AddCircle(new ImGui.ImVec2(x + sz.value * 0.5, y + sz.value * 0.5), sz.value * 0.5, col32, 20, curr_thickness);
                        x += sz.value + spacing;
                        draw_list.AddRect(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + sz.value, y + sz.value), col32, 0.0, ImGui.ImDrawCornerFlags.All, curr_thickness);
                        x += sz.value + spacing;
                        draw_list.AddRect(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + sz.value, y + sz.value), col32, 10.0, ImGui.ImDrawCornerFlags.All, curr_thickness);
                        x += sz.value + spacing;
                        draw_list.AddRect(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + sz.value, y + sz.value), col32, 10.0, ImGui.ImDrawCornerFlags.TopLeft | ImGui.ImDrawCornerFlags.BotRight, curr_thickness);
                        x += sz.value + spacing;
                        draw_list.AddTriangle(new ImGui.ImVec2(x + sz.value * 0.5, y), new ImGui.ImVec2(x + sz.value, y + sz.value - 0.5), new ImGui.ImVec2(x, y + sz.value - 0.5), col32, curr_thickness);
                        x += sz.value + spacing;
                        draw_list.AddLine(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + sz.value, y), col32, curr_thickness);
                        x += sz.value + spacing; // Horizontal line (note: drawing a filled rectangle will be faster!)
                        draw_list.AddLine(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x, y + sz.value), col32, curr_thickness);
                        x += spacing; // Vertical line (note: drawing a filled rectangle will be faster!)
                        draw_list.AddLine(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + sz.value, y + sz.value), col32, curr_thickness);
                        x += sz.value + spacing; // Diagonal line
                        draw_list.AddBezierCubic(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + sz.value * 1.3, y + sz.value * 0.3), new ImGui.ImVec2(x + sz.value - sz.value * 1.3, y + sz.value - sz.value * 0.3), new ImGui.ImVec2(x + sz.value, y + sz.value), col32, curr_thickness);
                        x = p.x + 4;
                        y += sz.value + spacing;
                    }
                    draw_list.AddCircleFilled(new ImGui.ImVec2(x + sz.value * 0.5, y + sz.value * 0.5), sz.value * 0.5, col32, 32);
                    x += sz.value + spacing;
                    draw_list.AddRectFilled(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + sz.value, y + sz.value), col32);
                    x += sz.value + spacing;
                    draw_list.AddRectFilled(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + sz.value, y + sz.value), col32, 10.0);
                    x += sz.value + spacing;
                    draw_list.AddRectFilled(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + sz.value, y + sz.value), col32, 10.0, ImGui.ImDrawCornerFlags.TopLeft | ImGui.ImDrawCornerFlags.BotRight);
                    x += sz.value + spacing;
                    draw_list.AddTriangleFilled(new ImGui.ImVec2(x + sz.value * 0.5, y), new ImGui.ImVec2(x + sz.value, y + sz.value - 0.5), new ImGui.ImVec2(x, y + sz.value - 0.5), col32);
                    x += sz.value + spacing;
                    draw_list.AddRectFilled(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + sz.value, y + thickness.value), col32);
                    x += sz.value + spacing; // Horizontal line (faster than AddLine, but only handle integer thickness)
                    draw_list.AddRectFilled(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + thickness.value, y + sz.value), col32);
                    x += spacing + spacing; // Vertical line (faster than AddLine, but only handle integer thickness)
                    draw_list.AddRectFilled(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + 1, y + 1), col32);
                    x += sz.value; // Pixel (faster than AddLine)
                    draw_list.AddRectFilledMultiColor(new ImGui.ImVec2(x, y), new ImGui.ImVec2(x + sz.value, y + sz.value), ImGui.IM_COL32(0, 0, 0), ImGui.IM_COL32(255, 0, 0), ImGui.IM_COL32(255, 255, 0), ImGui.IM_COL32(0, 255, 0));
                    ImGui.Dummy(new ImGui.ImVec2((sz.value + spacing) * 8, (sz.value + spacing) * 3));
                }
                ImGui.EndTabItem();
            }
            if (ImGui.BeginTabItem("Canvas")) {
                /* static */ const points = STATIC("points", new ImGui.ImVector());
                ...
                if (canvas_size.y < 50.0)
                    canvas_size.y = 50.0;
                draw_list.AddRectFilledMultiColor(canvas_pos, new ImGui.ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y), ImGui.IM_COL32(50, 50, 50), ImGui.IM_COL32(50, 50, 60), ImGui.IM_COL32(60, 60, 70), ImGui.IM_COL32(50, 50, 60));
                draw_list.AddRect(canvas_pos, new ImGui.ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y), ImGui.IM_COL32(255, 255, 255));
                let adding_preview = false;
                ImGui.InvisibleButton("canvas", canvas_size);
                const mouse_pos_in_canvas = new ImGui.ImVec2(ImGui.GetIO().MousePos.x - canvas_pos.x, ImGui.GetIO().MousePos.y - canvas_pos.y);
                ...
                draw_list.PushClipRect(canvas_pos, new ImGui.ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y), true); // clip lines within the canvas (if we resize it, etc.)
                for (let i = 0; i < points.value.Size - 1; i += 2)
                    draw_list.AddLine(new ImGui.ImVec2(canvas_pos.x + points.value.Data[i].x, canvas_pos.y + points.value.Data[i].y), new ImGui.ImVec2(canvas_pos.x + points.value.Data[i + 1].x, canvas_pos.y + points.value.Data[i + 1].y), ImGui.IM_COL32(255, 255, 0, 255), 2.0);
            ...
            if (ImGui.BeginTabItem("BG/FG draw lists")) {
                ...
                const window_center = new ImGui.ImVec2(window_pos.x + window_size.x * 0.5, window_pos.y + window_size.y * 0.5);
                if (draw_bg.value)
                    ImGui.GetBackgroundDrawList().AddCircle(window_center, window_size.x * 0.6, ImGui.IM_COL32(255, 0, 0, 200), 32, 10 + 4);
                if (draw_fg.value)
                    ImGui.GetForegroundDrawList().AddCircle(window_center, window_size.y * 0.6, ImGui.IM_COL32(0, 255, 0, 200), 32, 10);
                ImGui.EndTabItem();
            }
            ImGui.EndTabBar();
        }
    ...
    function ShowExampleAppDocuments(p_open) {
        ...
        if (!ImGui.Begin("Example: Documents", p_open, ImGui.ImGuiWindowFlags.MenuBar)) {
            ImGui.End();
            return;
        }
        // // Menu
        ...
        if (ImGui.TreeNode("0001: Renderer: Large Mesh Support")) {
            const draw_list = ImGui.GetWindowDrawList();
            {
                ...
                for (let n = 0; n < vtx_count.value / 4; n++) {
                    ...
                    const col = ImGui.IM_COL32(((n * 17) & 255), ((n * 59) & 255), ((n * 83) & 255), 255);
                    // draw_list->AddRectFilled(ImVec2(p.x + off_x, p.y + off_y), ImVec2(p.x + off_x + 50, p.y + off_y + 50), col);
                    draw_list.AddRectFilled(new ImGui.ImVec2(p.x + off_x, p.y + off_y), new ImGui.ImVec2(p.x + off_x + 50, p.y + off_y + 50), col);
                }
                ImGui.Dummy(new ImGui.ImVec2(300 + 50, 100 + 50));
            ...
            }
            {
                ...
                for (let n = 0; n < vtx_count.value / (10 * 4); n++) {
                    ...
                    draw_list.AddText(new ImGui.ImVec2(p.x + off_x, p.y + off_y), col, "ABCDEFGHIJ");
                }
                ImGui.Dummy(new ImGui.ImVec2(300 + 50, 100 + 20));
                // ImGui.Text("VtxBuffer.Size = %d", draw_list->VtxBuffer.Size);
                ImGui.Text(`VtxBuffer = ${draw_list.VtxBuffer.length}`);
            }
            ImGui.TreePop();
