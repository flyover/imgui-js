(function (global, factory) {
    ...
    class MemoryEditor {
        constructor() {
            ...
            this.DataInputBuf = new ImGui.ImStringBuffer(32, "");
            // char            AddrInputBuf[32];
            this.AddrInputBuf = new ImGui.ImStringBuffer(32, "");
            // size_t          GotoAddr;
            this.GotoAddr = -1;
        ...
        DrawWindow(title, mem_data, mem_size = mem_data.byteLength, base_display_addr = 0x000) {
            ...
            ImGui.SetNextWindowSizeConstraints(new ImGui.ImVec2(0.0, 0.0), new ImGui.ImVec2(s.WindowWidth, Number.MAX_VALUE));
            // this.Open = true;
            // if (ImGui.Begin(title, &Open, ImGuiWindowFlags_NoScrollbar))
            if (ImGui.Begin(title, (value = this.Open) => this.Open = value, ImGui.ImGuiWindowFlags.NoScrollbar)) {
                if (ImGui.IsWindowHovered(ImGui.ImGuiHoveredFlags.RootAndChildWindows) && ImGui.IsMouseClicked(1))
                    ImGui.OpenPopup("context");
                this.DrawContents(mem_data, mem_size, base_display_addr);
                if (this.ContentsWidthChanged) {
                    this.CalcSizes(s, mem_size, base_display_addr);
                    ImGui.SetWindowSize(new ImGui.ImVec2(s.WindowWidth, ImGui.GetWindowSize().y));
                }
            }
            ImGui.End();
        }
        ...
        DrawContents(mem_data, mem_size = mem_data.byteLength, base_display_addr = 0x0000) {
            ...
            ImGui.BeginChild("##scrolling", new ImGui.ImVec2(0, -footer_height_to_reserve));
            ...
            if (this.OptShowAscii)
                draw_list.AddLine(new ImGui.ImVec2(window_pos.x + s.PosAsciiStart - s.GlyphWidth, window_pos.y), new ImGui.ImVec2(window_pos.x + s.PosAsciiStart - s.GlyphWidth, window_pos.y + 9999), ImGui.GetColorU32(ImGui.ImGuiCol.Border));
            const color_text = ImGui.GetColorU32(ImGui.ImGuiCol.Text);
            const color_disabled = this.OptGreyOutZeroes ? ImGui.GetColorU32(ImGui.ImGuiCol.TextDisabled) : color_text;
            for (let line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
             {
                ...
                for (let n = 0; n < this.Rows && addr < mem_size; n++, addr++) {
                    ...
                    if ((addr >= this.HighlightMin && addr < this.HighlightMax) || (this.HighlightFn && this.HighlightFn(mem_data, addr))) {
                        ...
                        draw_list.AddRectFilled(pos, new ImGui.ImVec2(pos.x + highlight_width, pos.y + s.LineHeight), this.HighlightColor);
                    }
                ...
                if (this.OptShowAscii) {
                    ...
                    ImGui.PushID(line_i);
                    if (ImGui.InvisibleButton("ascii", new ImGui.ImVec2(s.PosAsciiEnd - s.PosAsciiStart, s.LineHeight))) {
                    ...
                    for (let n = 0; n < this.Rows && addr < mem_size; n++, addr++) {
                        if (addr === this.DataEditingAddr) {
                            draw_list.AddRectFilled(pos, new ImGui.ImVec2(pos.x + s.GlyphWidth, pos.y + s.LineHeight), ImGui.GetColorU32(ImGui.ImGuiCol.FrameBg));
                            draw_list.AddRectFilled(pos, new ImGui.ImVec2(pos.x + s.GlyphWidth, pos.y + s.LineHeight), ImGui.GetColorU32(ImGui.ImGuiCol.TextSelectedBg));
                        }
